...
int EwBspTouchInit( int aGuiWidth, int aGuiHeight, XDisplayInfo* aDisplayInfo )
{
  ...
  /* initialize the touch screen driver via BSP */
  ts_init.Width       = EW_TOUCH_AREA_MAX_X;           //DisplayWidth;
  ts_init.Height      = EW_TOUCH_AREA_MAX_Y;           //DisplayHeight;
...
{
  static TS_State_t    touchPadState;
  int                  touchX;
  int                  touchY;
  int                  x, y;
  int                  noOfEvents     = 0;
  ...
  /* avoid that the touch driver is read too fast again - limit to 10 ms */
  ...
  if ( ( touchPadState.TouchDetected ) && ( touchPadState.TouchX !=479 ) )
  {
    #ifdef EW_PRINT_TOUCH_DATA

       EwPrint( "Raw touch data: x %d, y %d\n", touchPadState.TouchX, touchPadState.TouchY );
    ...
    /* apply swapping of raw touch coordinates if required */
      ...
      touchX = touchPadState.TouchY;
      touchY = touchPadState.TouchX+60;

    #else

      touchX = touchPadState.TouchY+60;
      touchY = touchPadState.TouchX;
    #endif
