...
void GraphicsAttrString__Init( GraphicsAttrString _this, XObject aLink, XHandle aArg )
{
  ...
  /* Allow the Immediate Garbage Collection to evalute the members of this class. */
  _this->_.XObject._.GCT = EW_CLASS_GCT( GraphicsAttrString );

  /* Setup the VMT pointer */
  _this->_.VMT = EW_CLASS( GraphicsAttrString );
}
...
/* 'C' function for method : 'Graphics::AttrString.IsBaseDirectionRTL()' */
XBool GraphicsAttrString_IsBaseDirectionRTL( GraphicsAttrString _this )
{
  XHandle handle = _this->attrString;
  XBool result = 0;
  ...
  result = EwIsAttrTextRTL((XAttrString*)handle );
  return result;
}
...
void GraphicsAttrString_Parse( GraphicsAttrString _this, GraphicsAttrSet aAttrSet, 
  XString aString, XInt32 aWidth, XBool aEnableBidiText )
{
  XHandle attrSet;
  XHandle handle;
  XPoint s;
  ...
  handle = (XHandle)EwParseAttrString((XAttrSet*)attrSet, aString, aWidth, aEnableBidiText );
...
EW_DEFINE_CLASS( GraphicsAttrString, XObject, _.VMT, _.VMT, _.VMT, _.VMT, _.VMT, 
                 _.VMT, "Graphics::AttrString" )
...
void GraphicsAttrSet__Init( GraphicsAttrSet _this, XObject aLink, XHandle aArg )
{
  ...
  /* Allow the Immediate Garbage Collection to evalute the members of this class. */
  _this->_.XObject._.GCT = EW_CLASS_GCT( GraphicsAttrSet );

  /* Setup the VMT pointer */
  _this->_.VMT = EW_CLASS( GraphicsAttrSet );

  /* Call the user defined constructor */
  GraphicsAttrSet_Init( _this, aArg );
}
...
/* 'C' function for method : 'Graphics::AttrSet.Init()' */
void GraphicsAttrSet_Init( GraphicsAttrSet _this, XHandle aArg )
{
  /* Dummy expressions to avoid the 'C' warning 'unused argument'. */
  EW_UNUSED_ARG( aArg );

  if (( _this->bitmaps[ 0 ] != 0 ) && _this->bitmaps[ 0 ]->Mutable )
    EwAttachObjObserver( EwNewSlot( _this, GraphicsAttrSet_onUpdateBitmap ), (XObject)_this->bitmaps[ 
      0 ], 0 );

  if (( _this->fonts[ 0 ] != 0 ) && _this->fonts[ 0 ]->Mutable )
    EwAttachObjObserver( EwNewSlot( _this, GraphicsAttrSet_onUpdateFont ), (XObject)_this->fonts[ 
      0 ], 0 );
}
...
void GraphicsAttrSet_update( GraphicsAttrSet _this )
{
  ...
  for ( inx = 0; inx < 10; inx = inx + 1 )
  {
    ...
    if ( EwCompColor( entry, _Const0002 ))
...
/* 'C' function for method : 'Graphics::AttrSet.onUpdateBitmap()' */
void GraphicsAttrSet_onUpdateBitmap( GraphicsAttrSet _this, XObject sender )
{
  XHandle handle;
  XInt32 inx;
  ...
  handle = _this->attrSet;

  for ( inx = 0; inx < 10; inx = inx + 1 )
    if (( _this->bitmaps[ EwCheckIndex( inx, 10 )] != 0 ) && _this->bitmaps[ EwCheckIndex( 
        inx, 10 )]->Mutable )
    {
      XHandle bitmap = _this->bitmaps[ EwCheckIndex( inx, 10 )]->bitmap;
      EwSetAttrBitmap((XAttrSet*)handle, inx, (XBitmap*)bitmap );
    }
...
/* 'C' function for method : 'Graphics::AttrSet.onUpdateFont()' */
void GraphicsAttrSet_onUpdateFont( GraphicsAttrSet _this, XObject sender )
{
  XHandle handle;
  XInt32 inx;
  ...
  for ( inx = 0; inx < 4; inx = inx + 1 )
    if (( _this->fonts[ EwCheckIndex( inx, 4 )] != 0 ) && _this->fonts[ EwCheckIndex( 
        inx, 4 )]->Mutable )
    {
      XHandle font = _this->fonts[ EwCheckIndex( inx, 4 )]->font;
      EwSetAttrFont((XAttrSet*)handle, inx, (XFont*)font );
    }
  ...
  aFont )
{
  ...
  if ( fontNo < 4 )
  {
    if (( _this->fonts[ EwCheckIndex( fontNo, 4 )] != 0 ) && _this->fonts[ EwCheckIndex( 
        fontNo, 4 )]->Mutable )
      EwDetachObjObserver( EwNewSlot( _this, GraphicsAttrSet_onUpdateFont ), (XObject)_this->fonts[ 
        EwCheckIndex( fontNo, 4 )], 0 );

    modified = (XBool)( _this->fonts[ EwCheckIndex( fontNo, 4 )] != aFont );
    _this->fonts[ EwCheckIndex( fontNo, 4 )] = aFont;

    if (( aFont != 0 ) && aFont->Mutable )
      EwAttachObjObserver( EwNewSlot( _this, GraphicsAttrSet_onUpdateFont ), (XObject)aFont, 
        0 );
  }
  ...
  aBitmap )
{
  ...
  if ( bitmapNo < 10 )
  {
    ...
    if (( _this->bitmaps[ EwCheckIndex( bitmapNo, 10 )] != 0 ) && _this->bitmaps[ 
      ...
      EwDetachObjObserver( EwNewSlot( _this, GraphicsAttrSet_onUpdateBitmap ), (XObject)_this->bitmaps[ 
        EwCheckIndex( bitmapNo, 10 )], 0 );

    _this->bitmaps[ EwCheckIndex( bitmapNo, 10 )] = aBitmap;

    if (( aBitmap != 0 ) && aBitmap->Mutable )
      EwAttachObjObserver( EwNewSlot( _this, GraphicsAttrSet_onUpdateBitmap ), (XObject)aBitmap, 
        0 );
  }
...
EW_DEFINE_CLASS( GraphicsAttrSet, XObject, fonts, fonts, attrSet, attrSet, attrSet, 
                 attrSet, "Graphics::AttrSet" )
