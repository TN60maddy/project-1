...
#endif
#if(USE_TSC2007_TS_CTRL == 1U)
static int32_t TSC2007_Probe(uint32_t INstance);
#endif
...
//	  TSC2007_State_t state;
	  Ts_Drv->GetState(Ts_CompObj[Instance], TS_State);
...
static int32_t TSC2007_Probe(uint32_t Instance)
{
  int32_t ret = BSP_ERROR_NONE;
  TSC2007_IO_t IOCtx;
  static TSC2007_Object_t TSC2007Obj;
  uint32_t tsc2007_id;

  /* Configure the touch screen driver */
  IOCtx.Address          =  TS_TSC2007_I2C_ADDRESS;      // Set the I2C Address of the TSC2007
  IOCtx.Init                  =  BSP_I2C1_Init;                     // BSP_I2C3_Init;
  IOCtx.DeInit              =  BSP_I2C1_DeInit;                // BSP_I2C3_DeInit;
  IOCtx.ReadReg         =  BSP_I2C1_ReadReg;         //BSP_I2C3_ReadReg;
  IOCtx.WriteReg         =  BSP_I2C1_WriteReg;          // BSP_I2C3_WriteReg;
  IOCtx.GetTick           =  BSP_GetTick;                    //  BSP_GetTick;
  IOCtx.DevRed           = BSP_I2C1_IsReady;           //  BSP_I2C3_IsReady;

  if(TSC2007_RegisterBusIO(&TSC2007Obj, &IOCtx))
  {
	ret = BSP_ERROR_BUS_FAILURE;
  }

  else if (TSC2007_ReadID(&TSC2007Obj, &tsc2007_id) != TSC2007_OK)
  {
    ret = BSP_ERROR_COMPONENT_FAILURE;
  }

  // else if(tsc2007_id != TSC2007_ID)
  //{
    //ret = BSP_ERROR_UNKNOWN_COMPONENT;
  //}
  ...
  {
    Ts_CompObj[Instance] = &TSC2007Obj;
    Ts_Drv = (TS_Drv_t *) &TSC2007_TS_Driver;

    	if(Ts_Drv->Init(Ts_CompObj[Instance]) != TSC2007_OK)
    	{
    			ret = BSP_ERROR_COMPONENT_FAILURE;
    	}
  }
