...
static uint32_t SpiInitCounter = 0;
...
static int32_t SPI2_ReadReg(uint16_t Reg,uint8_t *pData, uint16_t Length);
static int32_t SPI2_WriteReg(uint16_t Reg,uint8_t *pData, uint16_t Length);
static int MX_SPI2_Init(void);
...
//    ret = BSP_ERROR_BUSY;
//  }
  if(HAL_I2C_IsDeviceReady(&hbus_i2c1, (uint16_t)(DevAddr << 1), 2, 100) != HAL_OK)
   {
     ret = BSP_ERROR_BUSY;
   }
  else
  {
	 ret = HAL_I2C_Mem_Write(&hbus_i2c1, (DevAddr << 1), 0x00, I2C_MEMADD_SIZE_8BIT, &data, 1, HAL_MAX_DELAY);

  }

  if(HAL_I2C_IsDeviceReady(&hbus_i2c1, (uint16_t)(TC_ID << 1), 2, 100) != HAL_OK)
    {
      ret = BSP_ERROR_BUSY;
    }
...
int32_t BSP_I2C3_IsReady(uint16_t DevAddr, uint32_t Trials)
{
  ...
  osSemaphoreWait(BspI2cSemaphore, osWaitForever);
#endif
  if(HAL_I2C_IsDeviceReady(&hbus_i2c3, (uint16_t)(DevAddr << 1), 2, 100) != HAL_OK)
  {
    ret = BSP_ERROR_BUSY;
  }
#if defined(BSP_USE_CMSIS_OS)
...
{
	if (HAL_I2C_Mem_Read(&hbus_i2c1, 0x90, Reg,1, pData, 2, 10000) == HAL_OK)
  {
    return BSP_ERROR_NONE;
  }
...
{
    if (HAL_I2C_Mem_Read(&hbus_i2c3,0x90, Reg, 1, pData, 2, 10000) == HAL_OK)
	 {
		 return BSP_ERROR_NONE;
	 }
     	 return BSP_ERROR_BUS_FAILURE;
...
/*SPI BUS Implementation for LCD Configuration(GC9503NP) */
...
hbus_spi2.Init.Mode = SPI_MODE_MASTER;
hbus_spi2.Init.Direction = SPI_DIRECTION_2LINES;
hbus_spi2.Init.DataSize = SPI_DATASIZE_9BIT;
hbus_spi2.Init.CLKPolarity = SPI_POLARITY_HIGH;
hbus_spi2.Init.CLKPhase = SPI_PHASE_1EDGE;
hbus_spi2.Init.NSS = SPI_NSS_SOFT;
hbus_spi2.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_128;
hbus_spi2.Init.FirstBit = SPI_FIRSTBIT_MSB;
hbus_spi2.Init.TIMode = SPI_TIMODE_DISABLE;
hbus_spi2.Init.CRCCalculation = SPI_CRCCALCULATION_DISABLE;
hbus_spi2.Init.CRCPolynomial = 0x0;
hbus_spi2.Init.NSSPMode = SPI_NSS_PULSE_ENABLE;
hbus_spi2.Init.NSSPolarity = SPI_NSS_POLARITY_LOW;
hbus_spi2.Init.FifoThreshold = SPI_FIFO_THRESHOLD_01DATA;
hbus_spi2.Init.TxCRCInitializationPattern = SPI_CRC_INITIALIZATION_ALL_ZERO_PATTERN;
hbus_spi2.Init.RxCRCInitializationPattern = SPI_CRC_INITIALIZATION_ALL_ZERO_PATTERN;
hbus_spi2.Init.MasterSSIdleness = SPI_MASTER_SS_IDLENESS_00CYCLE;
hbus_spi2.Init.MasterInterDataIdleness = SPI_MASTER_INTERDATA_IDLENESS_00CYCLE;
hbus_spi2.Init.MasterReceiverAutoSusp = SPI_MASTER_RX_AUTOSUSP_DISABLE;
hbus_spi2.Init.MasterKeepIOState = SPI_MASTER_KEEP_IO_STATE_DISABLE;
 hbus_spi2.Init.IOSwap = SPI_IO_SWAP_DISABLE;
if (HAL_SPI_Init(&hbus_spi2) != HAL_OK)
{
     //Error_Handler();
	ret = -1;
}
...
int32_t BSP_SPI2_ReadReg(uint16_t Reg, uint8_t *pData, uint16_t Length)
{
  ...
  osSemaphoreWait(BspI2cSemaphore, osWaitForever);
#endif
  if(SPI2_ReadReg(Reg,pData, Length) == 0)
  {
    ret = BSP_ERROR_NONE;
  }
  else
...
/**
  * @brief  Write a 8bit register of the device through BUS
  * @param  DevAddr Device address on BUS
  * @param  Reg     The target register address to read
  * @param  pData   Pointer to data buffer
  * @param  Length  Length of the data
...
int32_t BSP_SPI2_WriteReg(uint16_t Reg, uint8_t *pData, uint16_t Length)
{
  ...
  osSemaphoreWait(BspI2cSemaphore, osWaitForever);
#endif
  if(SPI2_WriteReg(Reg, pData, Length) == 0)
  {
    ret = BSP_ERROR_NONE;
  }
  else
...
{
	//EwBsp_GC9503_NSS(0);
     HAL_SPI_Transmit(&hbus_spi2,(uint8_t*)&Reg,1,100);
     if (HAL_SPI_Receive(&hbus_spi2,pData,1,100) == HAL_OK)
	 {
		 return BSP_ERROR_NONE;
	 }
     //EwBsp_GC9503_NSS(1);
...
static int32_t SPI2_WriteReg(uint16_t Reg,uint8_t *pData,uint16_t Length)
{
	 if (HAL_SPI_Transmit(&hbus_spi2,(uint8_t*)&Reg,Length,10000) == HAL_OK)
	 {
		 return BSP_ERROR_NONE;
	 }
     	 return BSP_ERROR_BUS_FAILURE;
