...
void EwBsp_GC9503_ResetPin_Init()
{
	  GPIO_InitTypeDef GPIO_InitStruct = {0};
	  /*ENABLE Clock for GPIO B Port */
	  __HAL_RCC_GPIOA_CLK_ENABLE();

      /*Configure GPIOB PIN5 as OUTPUT*/
...
/*void EwBsp_GPIO_Write(GPIO_TypeDef* GPIO_Port, int GPIO_Pin,int Val)
{
	HAL_GPIO_WritePin(GPIO_Port, GPIO_Pin, Val );
}*/


/*  GPIO Read Function */

/*void EwBsp_GPIO_Read(GPIO_TypeDef* GPIO_Port, int GPIO_Pin,int *Val)
{

	*Val = HAL_GPIO_ReadPin(GPIO_Port, GPIO_Pin);
}*/

void EwBsp_GC9503_Reset(int Val)
{
	HAL_GPIO_WritePin(GPIOA,GPIO_PIN_10, Val );
}

void EwBsp_GC9503_NSS(int Val)
{
	HAL_GPIO_WritePin(GPIOB,GPIO_PIN_5, Val );
}
