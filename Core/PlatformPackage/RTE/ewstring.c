...
/* The following variable exists for configuration purpose of the size of the
   invocation queue. See the function EwInvoke(). */
  ...
  int aUseReader )
{
  ...
  int                  bs    = !aUseReader? *(const int*)aBlock :
                               *(const int*)EwFlashAreaReaderProc( aBlock, 0 );
...
static int WcsLen2( const XChar* aString )
{
  ...
  /* Look for the zero-terminator sign */
  while ( *(const XChar*)EwFlashAreaReaderProc( tmp, 0 ))
    tmp++;
...
/* This internal function initializes the string module functionality. */
void EwInitStrings( void )
{
...
/* This internal function de-initializes the string module functionality. */
void EwDoneStrings( void )
{
  /* Dispose all existing strings */
  while ( Strings )
  {
    struct XStringStruct* tmp = Strings;
    Strings = tmp->Next;
  ...
  /* Free blocks with string constants */
  while ( CacheLast )
  {
    /* tmpS should refer to the control data of the affected string */
    struct XStringBlock* tmp = CacheLast;
    CacheLast = tmp->Prev;
...
XString EwLoadString( const XStringRes* aStringConst )
{
  ...
  /* Is the string data stored in compressed format? Uncompressed data starts
     with two 0xFFFF words. */
  if ( useReader )
  {
    compressed |= *(const XChar*)EwFlashAreaReaderProc( ptr,     0 ) != 0xFFFF;
    compressed |= *(const XChar*)EwFlashAreaReaderProc( ptr + 1, 0 ) != 0xFFFF;
  }
  else
  {
    compressed |= *(const XChar*)( ptr     ) != 0xFFFF;
    compressed |= *(const XChar*)( ptr + 1 ) != 0xFFFF;
  }
...
*
*******************************************************************************/
XString EwStringFormat( XString aFormat, XString aArg1, XString aArg2,
  XString aArg3, XString aArg4, XString aArg5, XString aArg6, XString aArg7,
  XString aArg8, XString aArg9 )
{
  XChar*  ptr1  = VerifyString( aFormat );
  XChar*  ptr2  = WcsChr( ptr1, '{' );
  int     size  = 0;
  XString str;

  /* First pass: Estimate the total length of the resulting string */
  while ( ptr2 )
  {
    XChar   digit = ptr2[1];
    XString par;

    /* Is it a valid placeholder? If not continue the search operation */
    if (( digit < '1' ) || ( digit > '9' ) || ( ptr2[2] != '}' ))
    {
      ptr2 = WcsChr( ptr2 + 1, '{' );
      continue;
    }

    /* Count the string preceding the placeholder */
    size += ptr2 - ptr1;

    /* Now get the string for the corresponding placeholder */
    switch ( digit )
    {
      case '1' : par = aArg1; break;
      case '2' : par = aArg2; break;
      case '3' : par = aArg3; break;
      case '4' : par = aArg4; break;
      case '5' : par = aArg5; break;
      case '6' : par = aArg6; break;
      case '7' : par = aArg7; break;
      case '8' : par = aArg8; break;
      case '9' : par = aArg9; break;
      default  : par = 0;
    }

    /* Count the length of the string provided in the parameter */
    size += WcsLen( VerifyString( par ));

    /* Continue with the text behind the placeholder */
    ptr1 = ptr2 + 3;
    ptr2 = WcsChr( ptr1, '{' );
  }

  /* Count remaining string after the last placeholder */
  size += WcsLen( ptr1 );
  ...
  /* Second pass: Compose the string */
  while ( ptr2 )
  {
    XChar   digit = ptr2[1];
    XString par;
    ...
    /* Copy the string preceding the placeholder */
    WcsCpy( str + size, ptr1, ptr2 - ptr1 );
    size += ptr2 - ptr1;
    ...
    /* Append the string provided in the parameter */
    WcsCpy( str + size, par, WcsLen( par ));
    size += WcsLen( par );
  ...
  /* Append the remaining string after the last placeholder */
  WcsCpy( str + size, ptr1, WcsLen( ptr1 ));
...
{
  const XVariantOfString* data = EwGetVariantOf( aConstant, XVariantOfString );

  /* The content of the constant is available directly in the binary. */
  if ( data->LangId != EW_BLOB_LANG_ID )
    return EwLoadString( &data->Value );

  /* The constant should be available in a BLOB file. */
  else
  {
    #ifdef EW_USE_BLOB_DATA
      const XBlobStringRes* v = (const XBlobStringRes*)EwGetBlobConstant(
                                (const XBlobLink*)data, EW_BLOB_TYPE_STRING );

      XStringRes            res;

      /* No matching constant in the BLOB found. */
      if ( !v )
        return 0;

      res.Block  = EwBlobStartAddress + v->Block;
      res.Offset = v->Offset;

      return EwLoadString( &res );
    #else
      EW_UNUSED_ARG( aConstant );
      EwError( 634 );
      return 0;
    #endif
  }
...
{
  const XVariantOfChar* data = EwGetVariantOf( aConstant, XVariantOfChar );
  ...
  {
    const XChar* v = (const XChar*)EwGetBlobConstant((const XBlobLink*)data,
                                                      EW_BLOB_TYPE_CHAR );
