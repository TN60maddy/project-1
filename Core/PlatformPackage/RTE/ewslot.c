  ...
  XObject aSender )
{
  ...
  /* If the found signal is already delivered and it has posted itself too
     often, discard the new signal. Otherwise endless processing will be
     possible */
  if ( *aList && (*aList)->Processed && ((*aList)->Repetition > 2 ))
    return 0;

  /* How often did the signal post itself already? */
  if ( *aList && (*aList)->Processed )
    repetition = (*aList)->Repetition + 1;

  /* If not yet processed, remove the found signal from the list (it will
  ...
  /* Or, if the affected signal is still not stored in the list - create and 
     initialize a new XPendingSignal object. This is also the case of a signal
     repetition, when the signal posts itself again. This is allowed for max.
     3 times */
  else
  {
...
/* This function removes all observers from the given list. */
static void ClearObservers( struct XObserver** aList )
{
...
void EwDoneSlots( void )
{
  ClearSignals  ( &PostSignals );
  ClearSignals  ( &IdleSignals );
  ClearObservers( &Observers );
  ClearObservers( &ObjObservers );
  ClearObservers( &RefObservers );
}
...
void EwNotifyObjObservers( XObject aObject, XUInt32 aId )
{
  ...
  while ( obs )
  {
    ...
    if (
      ...
      AddSignal( &PostSignals, obs->Slot, 0 );
