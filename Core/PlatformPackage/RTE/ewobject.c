...
static XObject AppRootObject = 0;
...
/* Internal functions to initialize and shutdown the functionality of Runtime
   Environment. */
extern void EwInitTimers( void );
extern void EwDoneTimers( void );
extern void EwInitStrings( void );
extern void EwDoneStrings( void );
extern void EwInitSlots( void );
extern void EwDoneSlots( void );
extern void EwInitResources( void );
extern void EwDoneResources( void );
extern void EwInitDebug( void );
extern void EwDoneDebug( void );
...
/* The following function frees all actually cached but not used surfaces in
   ...
   be freed. */
extern int EwCleanSurfaceCache( int aFullClean );
...
/* These global variables refer the memory area used by the CPU stack of the
   task running Embedded Wizard application. This information is used to verify
   whether the application is running out the stack memory. */
void** EwStackStartAddress = 0;
void** EwStackEndAddress   = 0;
...
/* These global variables store the number of bytes occupied by resources 
...
/* This variable stores the max. usage of the invocation queue */
int EwInvocationsMemory     = 0;
int EwInvocationsMemoryPeak = 0;
...
int EwInitRuntimeEnvironment( void* aArgs )
{
  ...
  ProfilerChain           = 0;
  ProfilerMemory          = 0;
  EwStackStartAddress     = 0;
  EwStackEndAddress       = 0;
  EwObjectsMemory         = 0;
  EwObjectsMemoryPeak     = 0;
  EwNoOfObjects           = 0;
  EwStringsMemory         = 0;
  EwStringsMemoryPeak     = 0;
  EwNoOfStrings           = 0;
  EwResourcesMemory       = 0;
  EwResourcesMemoryPeak   = 0;
  EwResourcesEvalMemory   = 0;
  EwOffScreenMemory       = 0;
  EwOffScreenMemoryPeak   = 0;
  EwInvocationsMemory     = 0;
  EwInvocationsMemoryPeak = 0;
  EwMemoryPeak            = 0;
  AutoObjects             = 0;
...
*   before invoking EwInitHeap() and after invoking EwInitInvocations() and
*   EwInitGraphicsEngine().
*
* ARGUMENTS:
*   None
...
void EwDoneRuntimeEnvironment( void )
{
  ...
  /* Unlock all objects */
  while ( RootSetHead )
    EwUnlockObject( RootSetHead->Object );

  /* Free all autoobjects */
  while ( AutoObjects )
  {
    XAutoObjectEntry* tmp = AutoObjects;
    AutoObjects = tmp->Next;
    EwFree( tmp );
  ...
  /* Free all still existing objects */
  while ( Objects )
  {
    XObjectFrame* frame  = Objects;
    XObject       object = (XObject)( frame + 1 );
...
{
  /* Does the stack grow up to lower address direction? */
  if ( aStackBaseAddress < aStackEndAddress )
  {
    EwStackStartAddress = (void**)aStackBaseAddress;
    EwStackEndAddress   = (void**)aStackEndAddress;
  }

  /* .. or higher addresses? */
  else
  {
    EwStackStartAddress = (void**)aStackEndAddress;
    EwStackEndAddress   = (void**)aStackBaseAddress;
  }
...
*
*******************************************************************************/
void EwSetAppRootObject( XObject aObject )
{
  AppRootObject = aObject;
}
...
*
*******************************************************************************/
XObject EwGetAppRootObject( void )
{
  return AppRootObject;
}
    ...
    for ( ; current != (XObject)1; current = current->_.Mark )
    {
      ...
      for ( ; vmt && ( vmt->_MagicNo == mno ); vmt = vmt->_Ancestor )
      {
        ...
        if ( gcInfo[0] && ( gcInfo[0] != gcInfo[1]))
        {
          ...
          char* objs   = start + gcInfo[0];
          char* end    = start + endOfs;

          /* Evaluate all embedded objects. Note an object has the size of at least
             XObject. If it is smaller, then ignore it. It is C Compiler padding */
          while ((( objs + sizeof( struct _obj_XObject )) <= end ) &&
                ((XObject)objs)->_.GCT )
          {
        ...
        if ( gcInfo[1] && ( gcInfo[1] != gcInfo[2]))
        {
          ...
          XObject* ptrs   = (XObject*)(void*)( start + gcInfo[1]);
        ...
        if ( gcInfo[2] && ( gcInfo[2] != gcInfo[3]))
        {
          ...
          XSlot* slots  = (XSlot*)(void*)( start + gcInfo[2]);
        ...
        if ( gcInfo[3] && ( gcInfo[3] != gcInfo[4]))
        {
          ...
          XRef* refs   = (XRef*)(void*)( start + gcInfo[3]);
  ...
  while ( *objects )
  {
    ...
    if ( !object->_.Mark )
    {
      ...
      *objects = frame->Next;
      ...
      /* Discarding the application root object? */
      if ( object == AppRootObject )
        AppRootObject = 0;
...
/* The following functions are used internally during immediate garbage 
  ...
  EW_NO_INLINE static int ImmediateReclaimMemoryIntern2( void* aStackAddress )
  {
    ...
    if ( startPtr < StackBaseAddress )
      endPtr   = (void**)((unsigned long long)StackBaseAddress & 
                           ~( sizeof( void* ) - 1 ));
    else
    {
      if ( startPtr > endPtr )
      ...
      startPtr = (void**)((unsigned long long)StackBaseAddress & 
                           ~( sizeof( void* ) - 1 ));
    }
...
void EwReconstructObjects( void )
{
  ...
  /* Process all existing auto objects */
  for ( entry = AutoObjects; entry; entry = entry->Next )
    entry->Origin->ReInit( entry->Object );
}
...
void EwPrintProfilerStatistic( int aDetailed )
{
  ...
  if (( EwInvocationsMemoryPeak > 0 ) || ( EwInvocationsMemory > 0 ))
  {
    /* Track the peak value */
    if ( EwInvocationsMemory > EwInvocationsMemoryPeak )
      EwInvocationsMemoryPeak = EwInvocationsMemory;

    EwPrint( "%8d bytes occupied in the invocation queue (peak %d bytes)\n",
             EwInvocationsMemory, EwInvocationsMemoryPeak );
