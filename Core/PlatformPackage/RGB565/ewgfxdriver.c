...
*
*******************************************************************************/
void* EwCreateIndex8Surface( int aWidth, int aHeight )
{
...
*
*******************************************************************************/
void* EwCreateConstIndex8Surface( int aWidth, int aHeight,
...
*
*******************************************************************************/
void EwDestroyIndex8Surface( void* aSurface )
{
...
*
*******************************************************************************/
int EwGetIndex8SurfaceMemory( void* aSurface, int aX, int aY,
...
*
*******************************************************************************/
void* EwCreateAlpha8Surface( int aWidth, int aHeight )
{
...
*
*******************************************************************************/
void* EwCreateConstAlpha8Surface( int aWidth, int aHeight,
  XSurfaceMemory* aMemory )
{
...
*
*******************************************************************************/
void EwDestroyAlpha8Surface( void* aSurface )
{
...
*
*******************************************************************************/
int EwGetAlpha8SurfaceMemory( void* aSurface, int aX, int aY, 
  int aIndex, int aWriteAccess, XSurfaceMemory* aMemory )
{
  XAlpha8Surface* surface = (XAlpha8Surface*)aSurface;
...
*
*******************************************************************************/
void EwInitSingleColorGradient( unsigned int aColor, XGradient* aGradient )
{
  unsigned int mask = 0xFF;

  aGradient->IsHorizontal = 0;
  aGradient->IsVertical   = 0;
  aGradient->InvWidth     = 0;
  aGradient->InvHeight    = 0;
  aGradient->Width        = 0;
  aGradient->Height       = 0;

  /* Calculate the components for the red channel */
  aGradient->R0 = ( aColor & mask ) << 20;
  aGradient->R1 = 0;
  aGradient->R2 = 0;
  aGradient->R3 = 0;
  ...
  int* aFirst )
{
  ...
  while ( *aPaths > 0 )
  {
    ...
    for ( aPaths += 4; aPaths <= stop; aPaths += 2, y1 = y2 )
    {
      ...
      /* Get the x coordinates of the affected edge. Note the +2 offset. The 
         algorithm calculates with coordinates in 16 sub-pixel precision. For
         the rasterization the results of the calculation are reduced to 4 sub
         pixel precision. For this purpose the values are divided by 4. The
         disadvantage of the division is the 'flooring'. This rounding error
         produces a slight left shift in the resulting graphic. To compensate
         this error we shift all coordinates by the half of a sub-pixel. One
         sub-pixel in the rasterized graphic corresponds to 4 sub-pixel in the
         source coordinates. The half of the sub-pixel is thus 2. */
      x1 = aPaths[-3] - aX + 2;
      x2 = aPaths[-1] - aX + 2;
  ...
  int* aFirst )
{
  ...
  while ( *aPaths > 0 )
  {
    ...
    for ( aPaths += 4; aPaths <= stop; aPaths += 2, y1 = y2 )
    {
      ...
      /* Get the x coordinates of the affected edge. Note the +4 offset. The 
         algorithm calculates with coordinates in 16 sub-pixel precision. For
         the rasterization the results of the calculation are reduced to one
         pixel precision. For this purpose the values are divided by 16. The
         disadvantage of the division is the 'flooring'. This rounding error
         produces a left shift in the resulting graphic. To compensate this
         error we shift all coordinates by the quarter of a pixel. */
      x1 = aPaths[-3] - aX + 4;
      x2 = aPaths[-1] - aX + 4;
  ...
  int aClipX2, int aClipY2, XGradient* aGradient, XWarpWorker aWorker )
{
  ...
  for ( y = y1 + 4; y < y2; y += 8 )
  {
    ...
    /* Check the source surface coordinates mapped to the left edge of the actual
       pixel row. If necessary discard leading pixels if they address outside the
       source surface. it may occure when large surface is strongly distorted. */
    while ( !affine && ( c > 0 ))
    {
      int w1s = ( w1 >> 12 )? w1 : 1 << 12;
      int tt  = ( t1 + ( w1s >> 13 )) / ( w1s >> 12 );
      int yy  = ( tt * aSrcHeight ) >> 8;

      /* Do the coordinates address inside the source surface? Then everything
         is o.k. Note the sub-pixel precision of 16 sub-pixels and the 1-pixel
         thick transparent border surrounding each frame */
      if (( yy > -7 ) && (( yy + 8 ) <= ( aSrcHeight << 4 )))
        break;

      /* Disclose the first pixel and adjust the coefficents to map to the next
         pixel of the row */
      c  -= 1;
      t1 += t2;
      s1 += s2;
      w1 += w2;
      dst.Pixel1 = (char*)dst.Pixel1 + dst.Pitch1X;
      dst.Pixel2 = (char*)dst.Pixel2 + dst.Pitch2X;
    }

    /* Check the source surface coordinates mapped to the right edge of the actual
       pixel row. If necessary discard trailing pixels if they address outside the
       source surface. It may occure when large surface is strongly distorted. */
    while ( !affine && ( c > 0 ))
    {
      int ww  = w1 + ( w2 * c );
      int wws = ( ww >> 12 )? ww : 1 << 12;
      int tt  = ( t1 + ( t2 * c ) + ( wws >> 13 )) / ( wws >> 12 );
      int yy  = ( tt * aSrcHeight ) >> 8;
      ...
      /* Disclose the last pixel */
      c -= 1;
    }

    /* Entire raw was eliminated in the above loops */
    if ( c <= 0 )
      continue;
...
*     filter operation. This property can take values in range of 0 .. 255.
...
*
*******************************************************************************/
void EwEmulateMaskTintFilter( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aDstX, int aDstY, int aWidth, int aHeight,
  int aSrcX, int aSrcY, int aMaskX, int aMaskY, int aMaskX1, int aMaskY1,
  int aMaskX2, int aMaskY2, int aMaskInverted, XGradient* aColor, 
  int aIntensity, int aOpacity, int aBlend, XMaskTintWorker aWorker1, 
  XMaskTintWorker aWorker2 )
{
  ...
  void* savedSrc1 = aSrc->Pixel1;
  void* savedSrc2 = aSrc->Pixel2;
  void* savedMask = aMask? aMask->Pixel1 : 0;
  ...
  /* Relative mask position to start the transfer */
  if ( aMask )
    aMask->Pixel1 = (char*)aMask->Pixel1 + aMaskY * aMask->Pitch1Y + 
                                           aMaskX * aMask->Pitch1X;

  /* Repeat until all rows have been processed */
  for ( ; aHeight > 0; aHeight--, aMaskY++ )
  {
    /* Can ignore the mask for the entire row? */
    if ( !aMask || ((( aMaskY < aMaskY1 ) || ( aMaskY >= aMaskY2 )) && 
          aMaskInverted ))
      aWorker2( aDst, aSrc, 0, aWidth, 0, aColor, aIntensity, aOpacity,
                 aBlend );

    /* Use the mask for the entire row? */
    else if (( aMaskX >= aMaskX1 ) && (( aMaskX + aWidth ) <= aMaskX2 ))
      aWorker1( aDst, aSrc, aMask, aWidth, aMaskInverted, aColor,
                aIntensity, aOpacity, aBlend );

    /* The mask is smaller than the row. Thus split the row in up to three
       fragments to perform the operation without and with the mask. */
    else
    {
      void* savedDst1R = aDst ->Pixel1;
      void* savedDst2R = aDst ->Pixel2;
      void* savedSrc1R = aSrc ->Pixel1;
      void* savedSrc2R = aSrc ->Pixel2;
      void* savedMaskR = aMask->Pixel1;

      int w1 = MIN( aWidth, MAX( aMaskX1 - aMaskX, 0 ));
      int w3 = MIN( aWidth, MAX( aMaskX  + aWidth - aMaskX2, 0 ));
      int w2 = aWidth - w1 - w3;

      /* Any pixel on the left of the mask to perform? */
      if ( w1 )
      {
        aWorker2( aDst, aSrc, 0, w1, 0, aColor, aIntensity, aOpacity, aBlend );

        /* Adjust the position for the row fragment */
        aDst ->Pixel1 = (char*)aDst ->Pixel1 + w1 * aDst ->Pitch1X;
        aDst ->Pixel2 = (char*)aDst ->Pixel2 + w1 * aDst ->Pitch2X;
        aSrc ->Pixel1 = (char*)aSrc ->Pixel1 + w1 * aSrc ->Pitch1X;
        aSrc ->Pixel2 = (char*)aSrc ->Pixel2 + w1 * aSrc ->Pitch2X;
        aMask->Pixel1 = (char*)aMask->Pixel1 + w1 * aMask->Pitch1X;
      }

      /* The row fragment intersecting the mask. */
      if ( w2 > 0 )
      {
        aWorker1( aDst, aSrc, aMask, w2, 1, aColor, aIntensity, aOpacity,
                  aBlend );

        /* Adjust the position for the row fragment */
        aDst ->Pixel1 = (char*)aDst ->Pixel1 + w2 * aDst ->Pitch1X;
        aDst ->Pixel2 = (char*)aDst ->Pixel2 + w2 * aDst ->Pitch2X;
        aSrc ->Pixel1 = (char*)aSrc ->Pixel1 + w2 * aSrc ->Pitch1X;
        aSrc ->Pixel2 = (char*)aSrc ->Pixel2 + w2 * aSrc ->Pitch2X;
      }

      /* Any pixel on the right of the mask to perform? */
      if ( w3 )
        aWorker2( aDst, aSrc, 0, w3, 0, aColor, aIntensity, aOpacity, aBlend );

      /* Restore the pointers */
      aDst ->Pixel1 = savedDst1R;
      aDst ->Pixel2 = savedDst2R;
      aSrc ->Pixel1 = savedSrc1R;
      aSrc ->Pixel2 = savedSrc2R;
      aMask->Pixel1 = savedMaskR;
    }
    ...
    if ( aMask )
      aMask->Pixel1 = (char*)aMask->Pixel1 + aMask->Pitch1Y;
  }
  ...
  aSrc->Pixel1 = savedSrc1;
  aSrc->Pixel2 = savedSrc2;
  if ( aMask ) aMask->Pixel1 = savedMask;
}
...
*     operation. This property can take values in range of 0 .. 255.
...
*
*******************************************************************************/
void EwEmulateBlurFilter( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  int aDstX, int aDstY, int aWidth, int aHeight, int aSrcX, int aSrcY,
  int aSrcX1, int aSrcX2, int aClamp1, int aClamp2, int aRadius, int aOpacity,
  int aBlend, XBlurWorker aWorker )
{
  void* savedDst1 = aDst->Pixel1;
  void* savedDst2 = aDst->Pixel2;
  void* savedSrc1 = aSrc->Pixel1;
  void* savedSrc2 = aSrc->Pixel2;
  int   limit1    = aSrcX1   - aSrcX;
  int   limit2    = aSrcX2   - aSrcX;

  /* Avoid clamping within the source surface area */
  aClamp1 = MAX( aClamp1, -limit1 );
  aClamp2 = MAX( aClamp2,  limit2 );
  ...
  /* Repeat until all rows have been processed. Note, the rows are stored as
     columns in the destination. The rows and columns are exchanged during
     this operation. */
  while ( aWidth-- > 0 )
  {
    aWorker( aDst, aSrc, aHeight, limit1, limit2, aClamp1, aClamp2, aRadius,
             aOpacity, aBlend );

    /* Continue with the next row in the source and next column in the 
       destination surfaces */
    aDst->Pixel1 = (char*)aDst->Pixel1 + aDst->Pitch1X;
    aDst->Pixel2 = (char*)aDst->Pixel2 + aDst->Pitch2X;
    aSrc->Pixel1 = (char*)aSrc->Pixel1 + aSrc->Pitch1Y;
    aSrc->Pixel2 = (char*)aSrc->Pixel2 + aSrc->Pitch2Y;
  }
...
*
*******************************************************************************/
void EwEmulateMaskTintBlurFilter( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, XSurfaceMemory* aAux, int aDstX, int aDstY, int aWidth,
  int aHeight, int aSrcX, int aSrcY, int aSrcX1, int aSrcX2, int aMaskX, 
  int aMaskY, int aMaskX1, int aMaskY1, int aMaskX2, int aMaskY2, 
  int aMaskInverted, XGradient* aColor, int aIntensity, int aClamp1,
  int aClamp2, int aRadius, XCopyWorker aWorker0, XMaskTintWorker aWorker1, 
  XMaskTintWorker aWorker2, XBlurWorker aWorker3, XBlurWorker aWorker4 )
{
  void*     savedDst1 = aDst->Pixel1;
  void*     savedDst2 = aDst->Pixel2;
  void*     savedSrc1 = aSrc->Pixel1;
  void*     savedSrc2 = aSrc->Pixel2;
  void*     savedAux1 = aAux->Pixel1;
  void*     savedAux2 = aAux->Pixel2;
  void*     savedMask = aMask? aMask->Pixel1 : 0;
  int       limit1    = aSrcX1  - aSrcX;
  int       limit2    = aSrcX2  - aSrcX;
  int       maxWidth  = aSrcX2  - aSrcX1;
  XGradient white;

  /* Prepare the color as XGradient structure */
  EwInitSingleColorGradient( 0xFFFFFFFF, &white );
  ...
  /* Relative position within the auxiliary surface to start the transfer.
     The position has to correspond to the position within a source surface
     row */
  aAux->Pixel1 = (char*)aAux->Pixel1 + aSrcX * aAux->Pitch1X;
  aAux->Pixel2 = (char*)aAux->Pixel2 + aSrcX * aAux->Pitch2X;
  ...
  /* The blur algorithm calculates with source pixel lying beyond the original
     source area. When we plan to mask/tint the source we have thus to also
     mask/tint those additional pixel. We have to enhance the amount of pixel
     which are masked/tinted. Adjust the pointers accordingly. Note: the total
     area to take in account is determined by aSrcX1..aSrcX2 */
  if ( aMask )
  {
    aMask->Pixel1 = (char*)aMask->Pixel1 - ( aSrcX - aSrcX1 ) * aMask->Pitch1X;
    aMaskX       -= aSrcX - aSrcX1;
  }
  ...
  /* Repeat until all rows have been processed. Note, the rows are stored as
     columns in the destination. The rows and columns are exchanged during
     this operation. */
  for ( ; aWidth > 0; aWidth--, aMaskY++ )
  {
    XBlurWorker     blurWorker = aWorker3;
    XSurfaceMemory* blurSrc    = aAux;

    /* The blur algorithm calculates with source pixel lying beyond the original
       source area. When we plan to mask/tint the source we have thus to also
       mask/tint those additional pixel. We have to enhance the amount of pixel
       which are masked/tinted. Adjust the pointers accordingly. */
    aAux->Pixel1 = (char*)aAux->Pixel1 - ( aSrcX - aSrcX1 ) * aAux->Pitch1X;
    aAux->Pixel2 = (char*)aAux->Pixel2 - ( aSrcX - aSrcX1 ) * aAux->Pitch2X;
    aSrc->Pixel1 = (char*)aSrc->Pixel1 - ( aSrcX - aSrcX1 ) * aSrc->Pitch1X;
    aSrc->Pixel2 = (char*)aSrc->Pixel2 - ( aSrcX - aSrcX1 ) * aSrc->Pitch2X;

    /* No mask to apply. Only tint filter. */
    if ( !aMask )
      aWorker2( aAux, aSrc, 0, maxWidth, 0, aColor, aIntensity, 255, 0 );

    /* Can ignore the mask for the entire row? The row is thus affected by tint
       filter only. */
    else if ( aMaskInverted && (( aMaskY < aMaskY1 ) || ( aMaskY >= aMaskY2 )))
    {
      /* Should we tint the entire pixel row? */
      if ( aWorker2 )
        aWorker2( aAux, aSrc, 0, maxWidth, 0, aColor, aIntensity, 255, 0 );

       /* Neither mask nor tint to apply. We can skip over the copy operation
          into the auxiliary surface and blur directly the contents from the
          source surface. Select the right worker and source surface. */
      else
      {
        blurWorker = aWorker4;
        blurSrc    = aSrc;
    ...
    /* Use the mask/tint for the entire row? */
    else if (( aMaskX >= aMaskX1 ) && (( aMaskX + maxWidth ) <= aMaskX2 ))
      aWorker1( aAux, aSrc, aMask, maxWidth, aMaskInverted, aColor,
                aIntensity, 255, 0 );
    ...
    {
      void* savedAux1R = aAux ->Pixel1;
      void* savedAux2R = aAux ->Pixel2;
      void* savedSrc1R = aSrc ->Pixel1;
      void* savedSrc2R = aSrc ->Pixel2;
      void* savedMaskR = aMask->Pixel1;

      /* Calculate the three possible areas to split the row */
      int w1 = MIN( maxWidth, MAX( aMaskX1 - aMaskX, 0 ));
      int w3 = MIN( maxWidth, MAX( aMaskX  + maxWidth - aMaskX2, 0 ));
      int w2 = maxWidth - w1 - w3;

      /* Any pixel on the left of the mask to perform? */
      if ( w1 )
      {
        /* Should the pixel be tinted? */
        if ( aWorker2 && aMaskInverted )
          aWorker2( aAux, aSrc, 0, w1, 0, aColor, aIntensity, 255, 0 );

        /* There was only mask specified - no tint. The affected area lies
           beyond the borders of the inverted mask -> thus simply copy the
           pixel from the source to auxiliary surface. */
        else if ( aMaskInverted )
          aWorker0( aAux, aSrc, w1, &white );

        /* Adjust the position for the next row fragment */
        aAux ->Pixel1 = (char*)aAux ->Pixel1 + w1 * aAux ->Pitch1X;
        aAux ->Pixel2 = (char*)aAux ->Pixel2 + w1 * aAux ->Pitch2X;
        aSrc ->Pixel1 = (char*)aSrc ->Pixel1 + w1 * aSrc ->Pitch1X;
        aSrc ->Pixel2 = (char*)aSrc ->Pixel2 + w1 * aSrc ->Pitch2X;
        aMask->Pixel1 = (char*)aMask->Pixel1 + w1 * aMask->Pitch1X;
      }

      /* The row fragment intersecting the mask. Apply the mask/tint filter on
         it */
      if ( w2 > 0 )
      {
        aWorker1( aAux, aSrc, aMask, w2, aMaskInverted, aColor, aIntensity,
                  255, 0 );

        /* Adjust the position for the row fragment */
        aAux ->Pixel1 = (char*)aAux ->Pixel1 + w2 * aAux ->Pitch1X;
        aAux ->Pixel2 = (char*)aAux ->Pixel2 + w2 * aAux ->Pitch2X;
        aSrc ->Pixel1 = (char*)aSrc ->Pixel1 + w2 * aSrc ->Pitch1X;
        aSrc ->Pixel2 = (char*)aSrc ->Pixel2 + w2 * aSrc ->Pitch2X;
      }
      ...
      if ( w3 && aMaskInverted )
      {
        /* Should the pixel be tinted? */
        if ( aWorker2 )
          aWorker2( aAux, aSrc, 0, w3, 0, aColor, aIntensity, 255, 0 );

        /* There was only mask specified - no tint. The affected area lies
           beyond the borders of the inverted mask -> thus simply copy the
           pixel from the source to auxiliary surface. */
        else
          aWorker0( aAux, aSrc, w3, &white );
      }

      /* Restore the pointers */
      aAux ->Pixel1 = savedAux1R;
      aAux ->Pixel2 = savedAux2R;
      aSrc ->Pixel1 = savedSrc1R;
      aSrc ->Pixel2 = savedSrc2R;
      aMask->Pixel1 = savedMaskR;
    }

    /* Once the mask/tint filter is done, restore the original pointers again
       so the blur filter can be applied */
    aAux->Pixel1 = (char*)aAux->Pixel1 + ( aSrcX - aSrcX1 ) * aAux->Pitch1X;
    aAux->Pixel2 = (char*)aAux->Pixel2 + ( aSrcX - aSrcX1 ) * aAux->Pitch2X;
    aSrc->Pixel1 = (char*)aSrc->Pixel1 + ( aSrcX - aSrcX1 ) * aSrc->Pitch1X;
    aSrc->Pixel2 = (char*)aSrc->Pixel2 + ( aSrcX - aSrcX1 ) * aSrc->Pitch2X;
    ...
    /* Run the blur filter */
    blurWorker( aDst, blurSrc, aHeight, limit1, limit2, aClamp1, aClamp2, 
                aRadius, 255, 0 );
  ...
  aSrc->Pixel1 = savedSrc1;
  aSrc->Pixel2 = savedSrc2;
  aAux->Pixel1 = savedAux1;
  aAux->Pixel2 = savedAux2;
  if ( aMask ) aMask->Pixel1 = savedMask;
}
...
*   aRadius       - Blur radius in pixel.
*   aOpacity      - Modulates the opacity of the pixel data resulting from the
*     blur/mask/tint operation. This property can take values in range of 0 ..
*     255.
*   aBlend        - If != 0, the drawn pixel will be alpha-blended with the
*     pixel in the background.
...
*
*******************************************************************************/
void EwEmulateBlurMaskTintFilter( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, XSurfaceMemory* aAux, int aDstX, int aDstY,
  int aWidth, int aHeight, int aSrcX, int aSrcY, int aSrcX1, int aSrcX2,
  int aMaskX, int aMaskY, int aMaskX1, int aMaskY1, int aMaskX2, int aMaskY2,
  int aMaskInverted, XGradient* aColor, int aIntensity, int aClamp1,
  int aClamp2, int aRadius, int aOpacity, int aBlend, XBlurWorker aWorker1, 
  XBlurWorker aWorker2, XMaskTintWorker aWorker3, XMaskTintWorker aWorker4,
  XMaskTintWorker aWorker5 )
{
  void* savedDst1       = aDst->Pixel1;
  void* savedDst2       = aDst->Pixel2;
  int   savedDstPitch1X = aDst->Pitch1X;
  int   savedDstPitch2X = aDst->Pitch2X;
  void* savedSrc1       = aSrc->Pixel1;
  void* savedSrc2       = aSrc->Pixel2;
  void* savedAux1       = aAux->Pixel1;
  void* savedAux2       = aAux->Pixel2;
  int   savedAuxPitch1Y = aAux->Pitch1Y;
  int   savedAuxPitch2Y = aAux->Pitch2Y;
  void* savedMask       = aMask? aMask->Pixel1  : 0;
  int   savedMaskPitchX = aMask? aMask->Pitch1X : 0;
  int   limit1          = aSrcX1  - aSrcX;
  int   limit2          = aSrcX2  - aSrcX;
  ...
  /* Relative position within the auxiliary surface to start the transfer.
     The position has to correspond to the position within the destination
     surface column */
  aAux->Pixel1 = (char*)aAux->Pixel1 + aDstY * aAux->Pitch1X;
  aAux->Pixel2 = (char*)aAux->Pixel2 + aDstY * aAux->Pitch2X;
  ...
  /* During this operation we will access the destination column-wise. For this
     purpose modify the pitch information, so the worker when skipping to the
     next pixel in the destination, in fact skips to the next row. */
  aDst->Pitch1X = aDst->Pitch1Y;
  aDst->Pitch2X = aDst->Pitch2Y;

  /* ... the same in case of the mask surface */
  if ( aMask )
    aMask->Pitch1X = aMask->Pitch1Y;

  /* Similarly, the auxiliary surface has a row-layout. We need however, to
     access it both row-wise and column-wise. */
  aAux->Pitch1Y = aAux->Pitch1X;
  aAux->Pitch2Y = aAux->Pitch2X;

  /* Repeat until all rows/columns have been processed. Note, the source stores
     the pixel in rows. The rows are then copied as columns in the destination
     surface. The rows and columns are exchanged during this operation. */
  for ( ; aWidth > 0; aWidth--, aMaskX++ )
  {
    /* Performing a mask operation for inverted mask and the affected column
       lies beyond the borders of the mask and there is no tint filter to apply?
       Then we can store the blur results directly in the destination surface
       skipping over the transfers to/from the auxiliary surface. */
    if ( !aWorker4 && (( aMaskX < aMaskX1 ) || ( aMaskX >= aMaskX2 )))
      aWorker2( aDst, aSrc, aHeight, limit1, limit2, aClamp1, aClamp2, aRadius,
                aOpacity, aBlend );

    /* Combination of blur and mask/tint is needed */
    else
    {
      /* Blur a single row from the source surface and store the results within
         the auxiliary surface assuming it has the layout of a single column */
      aWorker1( aAux, aSrc, aHeight, limit1, limit2, aClamp1, aClamp2, aRadius,
                255, 0 );

      /* No mask to apply. Only tint filter. Use it for the entire column. */
      if ( aWorker4 && ( !aMask || (( aMaskX < aMaskX1 ) || ( aMaskX >= aMaskX2 ))))
        aWorker4( aDst, aAux, 0, aHeight, 0, aColor, aIntensity, aOpacity,
                  aBlend );

      /* Use the mask/tint for the entire column? */
      else if (( aMaskY >= aMaskY1 ) && (( aMaskY + aHeight ) <= aMaskY2 ))
        aWorker3( aDst, aAux, aMask, aHeight, aMaskInverted, aColor, aIntensity,
                  aOpacity, aBlend );

      /* The mask is smaller than the column. Thus split the column in up to 
         three fragments to perform the operation without and with the mask. */
      else if ( aMask )
      {
        void* savedDst1R = aDst ->Pixel1;
        void* savedDst2R = aDst ->Pixel2;
        ...
        int h1 = MIN( aHeight, MAX( aMaskY1 - aMaskY, 0 ));
        int h3 = MIN( aHeight, MAX( aMaskY  + aHeight - aMaskY2, 0 ));
        int h2 = aHeight - h1 - h3;

        /* Any pixel above the mask to perform? */
        if ( h1 )
        {
          /* Perform the transfer with tint filter? */
          if ( aWorker4 )
            aWorker4( aDst, aAux, 0, h1, 0, aColor, aIntensity, aOpacity, 
                      aBlend );

          /* No, just copy the pixel between the aAux and aDst */
          else
            aWorker5( aDst, aAux, 0, h1, 0, aColor, aIntensity, aOpacity, 
                      aBlend );

          /* Adjust the position for the column fragment. Note, the pitch values
             for destination and mask have been modified in the code above to
             indicate the distance between two rows. */
          aDst ->Pixel1 = (char*)aDst ->Pixel1 + h1 * aDst ->Pitch1X;
          aDst ->Pixel2 = (char*)aDst ->Pixel2 + h1 * aDst ->Pitch2X;
          aAux ->Pixel1 = (char*)aAux ->Pixel1 + h1 * aAux ->Pitch1X;
          aAux ->Pixel2 = (char*)aAux ->Pixel2 + h1 * aAux ->Pitch2X;
          aMask->Pixel1 = (char*)aMask->Pixel1 + h1 * aMask->Pitch1X;
        }

        /* The column fragment intersecting the mask. */
        if ( h2 > 0 )
        {
          aWorker3( aDst, aAux, aMask, h2, 1, aColor, aIntensity, aOpacity,
                    aBlend );

          /* Adjust the position for the column fragment. Note, the pitch value
             for destination has been modified in the code above to indicate the
             distance between two rows. */
          aDst ->Pixel1 = (char*)aDst ->Pixel1 + h2 * aDst ->Pitch1X;
          aDst ->Pixel2 = (char*)aDst ->Pixel2 + h2 * aDst ->Pitch2X;
          aAux ->Pixel1 = (char*)aAux ->Pixel1 + h2 * aAux ->Pitch1X;
          aAux ->Pixel2 = (char*)aAux ->Pixel2 + h2 * aAux ->Pitch2X;
        }

        /* Any pixel below the mask to perform? */
        if ( h3 )
        {
          /* Perform the transfer with tint filter? */
          if ( aWorker4 )
            aWorker4( aDst, aAux, 0, h3, 0, aColor, aIntensity, aOpacity,
                      aBlend );

          /* No, just copy the pixel between the aAux and aDst */
          else
            aWorker5( aDst, aAux, 0, h3, 0, aColor, aIntensity, aOpacity,
                      aBlend );
        }
    ...
    /* Continue with the next row in the source and next column in the 
       destination surfaces */
    aDst->Pixel1 = (char*)aDst->Pixel1 + savedDstPitch1X;
    aDst->Pixel2 = (char*)aDst->Pixel2 + savedDstPitch2X;
    aSrc->Pixel1 = (char*)aSrc->Pixel1 + aSrc->Pitch1Y;
    aSrc->Pixel2 = (char*)aSrc->Pixel2 + aSrc->Pitch2Y;

    /* Note: mask is applied in the last phase when transferring the content of
       a blurred column into the destination surface. The mask is also evaluated
       column-wise. For the next iteration skip thus to the next column of the
       mask. */
    if ( aMask )
      aMask->Pixel1 = (char*)aMask->Pixel1 + savedMaskPitchX;
  }
  ...
  aDst->Pixel1  = savedDst1;
  aDst->Pixel2  = savedDst2;
  aDst->Pitch1X = savedDstPitch1X;
  aDst->Pitch2X = savedDstPitch2X;
  aSrc->Pixel1  = savedSrc1;
  aSrc->Pixel2  = savedSrc2;
  aAux->Pixel1  = savedAux1;
  aAux->Pixel2  = savedAux2;
  aAux->Pitch1Y = savedAuxPitch1Y;
  aAux->Pitch2Y = savedAuxPitch2Y;
  if ( aMask ) aMask->Pixel1  = savedMask;
  if ( aMask ) aMask->Pitch1X = savedMaskPitchX;
}
...
*
*******************************************************************************/
void EwEmulateFillLinearGradient( XSurfaceMemory* aDst, int* aPaths, int aDstX,
  int aDstY, int aWidth, int aHeight, int aAntialiased, int aNonZeroWinding,
  int aStartX, int aStartY, int aEndX, int aEndY, int aNoOfStops,
  XGradientStop* aStops, XFillLinearGradientWorker aWorker )
{
    ...
    void*           savedDst2 = aDst->Pixel2;
    int             x         = aDstX << 4;
    XSurfaceMemory  src       = { 0, 1, 0, 0, 0, 0, 0 };
    int             dx        = aEndX - aStartX;
    int             dy        = aEndY - aStartY;
    float           len       = EwMathSqrt((float)(( dx * dx ) + ( dy * dy )));
    int             unitX     = (int)(( dx / len ) * 65536.0f );
    int             unitY     = (int)(( dy / len ) * 65536.0f );
    unsigned char*  buf;
    unsigned char*  pixel;
    unsigned char*  edgeBuf;
    void*           stops;
    ...
    do
      buf = EwAlloc( bufSize );
    while ( !buf && EwImmediateReclaimMemory( 69 ));
    ...
    do
      stops = EwAlloc( sizeof( int ) * 2 * aNoOfStops );
    while ( !stops && EwImmediateReclaimMemory( 70 ));

    /* We were not able to reserve memory for temporary stop list. Print just
       a warning and continue. The application should not enter panic in this
       case. */
    if ( !stops )
    {
      EwResourcesMemory -= sizeof( bufSize );
    ...
    /* Track the temporary used memory */
    EwResourcesMemory += sizeof( int ) * 2 * aNoOfStops;
    ...
    /* Temp. used memory for color stops */
    offsets = stops;
    colors  = (unsigned int*)( offsets + aNoOfStops );

    /* Prepare a list of gradient stops. The offsets are expressed as distances
       to the start position of the gradient in pixel. */
    for ( i = 0; i < aNoOfStops; i++ )
    {
      colors[i]  = aStops[i].Color;
      offsets[i] = (int)( aStops[i].Offset * len + 0.5f );
    }
    ...
    for ( ; aHeight > 0; aHeight--, aDstY++ )
    {
      ...
      if ( count )
      {
        /* Offsets relative between the first pixel to raster and the start
           position of the gradient */
        int dx = first + aDstX - aStartX;
        int dy = aDstY - aStartY;
        ...
        /* Perform the fill gradient operation using the rasterized pixel as
           mask */
        aWorker( aDst, &src, count, dx, dy, unitX, unitY, aNoOfStops, offsets,
                 colors );
    ...
    EW_UNUSED_ARG( aAntialiased );
    EW_UNUSED_ARG( aNonZeroWinding );
    EW_UNUSED_ARG( aStartX );
    EW_UNUSED_ARG( aStartY );
    EW_UNUSED_ARG( aEndX );
    EW_UNUSED_ARG( aEndY );
    EW_UNUSED_ARG( aNoOfStops );
    EW_UNUSED_ARG( aStops );
    EW_UNUSED_ARG( aWorker );
  #endif /* EW_DONT_USE_GRADIENTS && defined EW_DONT_USE_PATH_FUNCTIONS */
}
...
*
*******************************************************************************/
void EwEmulateFillRadialGradient( XSurfaceMemory* aDst, int* aPaths, int aDstX,
  int aDstY, int aWidth, int aHeight, int aAntialiased, int aNonZeroWinding,
  int aStartX, int aStartY, int aRadius, float* aMatrix, int aNoOfStops,
  XGradientStop* aStops, XFillRadialGradientWorker aWorker )
{
  #if !defined EW_DONT_USE_GRADIENTS && !defined EW_DONT_USE_PATH_FUNCTIONS
    int             bufSize   = aWidth * ( aNonZeroWinding? 9 : 
                                           aAntialiased? 5 : 2 );
    ...
    matrix[0] = (int)( aMatrix[0] * 65536.0f );
    matrix[1] = (int)( aMatrix[1] * 65536.0f );
    matrix[2] = (int)( aMatrix[2] * 65536.0f );
    matrix[3] = (int)( aMatrix[3] * 65536.0f );
    useMatrix = ( matrix[0] != matrix[3]) || ( matrix[1] != matrix[2]) ||
                ( matrix[0] != 65536    ) || ( matrix[1] != 0        );
    ...
    for ( ; aHeight > 0; aHeight--, aDstY++ )
    {
      ...
      if ( count )
      {
        ...
        /* Perform the fill gradient operation using the rasterized pixel as
           mask */
        aWorker( aDst, &src, count, dx, dy, useMatrix? matrix : 0, aNoOfStops,
                 offsets, colors );
