...
#if ( EW_COLOR_CHANNEL_BIT_OFFSET_RED   != 16 ) ||                             \
    ( EW_COLOR_CHANNEL_BIT_OFFSET_GREEN != 8  ) ||                             \
    ( EW_COLOR_CHANNEL_BIT_OFFSET_BLUE  != 0  ) ||                             \
...
#if ( EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_RED   != 11 ) ||                      \
    ( EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_GREEN != 5  ) ||                      \
    ( EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_BLUE  != 0  )
...
#define USE_FLASH_READER( aAddress )                                           \
  ...
  /* Helper macro to test whether the given magic number does refer a BLOB file
     content. */
  #define IS_BLOB_RES                                                          \
    (( res->MagicNo == EW_MAGIC_NO_BITMAP_BLOB      ) ||                       \
     ( res->MagicNo == EW_MAGIC_NO_BITMAP_BLOB_R90  ) ||                       \
     ( res->MagicNo == EW_MAGIC_NO_BITMAP_BLOB_R180 ) ||                       \
     ( res->MagicNo == EW_MAGIC_NO_BITMAP_BLOB_R270 ))
  ...
  #define RES_PTR( aMember )                                                   \
    ( isBlob?                                                                  \
    ( EwBlobStartAddress + ((const XBlobBmpRes*)res)->aMember ) :              \
    (const char*)res->aMember )
#else
  /* Helper macro to test whether the given magic number does refer a BLOB file
     content. */
  #define IS_BLOB_RES 0
...
static const char* GetResourceName( const XBmpRes* aResource );
...
*
*******************************************************************************/
void* EwBmpOpen( const struct XBmpRes* aResource )
{
  const XBmpRes* res    = aResource;
  int            isBlob = IS_BLOB_RES;
  ...
  {
    EwErrorS( 234, GetResourceName( aResource ));
    EwPanic();
    return 0;
  }
  ...
  /* Verify whether the orientation of the bitmap does correspond to the used
    ...
    if (( aResource->MagicNo != EW_MAGIC_NO_BITMAP_R90 ) &&
        ( aResource->MagicNo != EW_MAGIC_NO_BITMAP_BLOB_R90 ))
  #elif EW_SURFACE_ROTATION == 180
    if (( aResource->MagicNo != EW_MAGIC_NO_BITMAP_R180 ) &&
        ( aResource->MagicNo != EW_MAGIC_NO_BITMAP_BLOB_R180 ))
  #elif EW_SURFACE_ROTATION == 270
    if (( aResource->MagicNo != EW_MAGIC_NO_BITMAP_R270 ) &&
        ( aResource->MagicNo != EW_MAGIC_NO_BITMAP_BLOB_R270 ))
  #else
    if (( aResource->MagicNo != EW_MAGIC_NO_BITMAP ) &&
        ( aResource->MagicNo != EW_MAGIC_NO_BITMAP_BLOB ))
  #endif
  {
    EwErrorS( 235, GetResourceName( aResource ));
    EwPanic();
    return 0;
  }
  ...
  {
    const unsigned int* pixel = (const unsigned int*)RES_PTR( Pixel1 );

    /* If the memory of constant surfaces is not accessible by the CPU, use a
       'Reader' to load and map the pixel pointers.  */
    #ifdef EW_USE_READER_FOR_CONST_SURFACES
      pixel = (const unsigned int*)EwFlashAreaReaderProc( pixel, 0 );
    #endif

    if ( pixel[0] != EW_COLOR_CHANNELS_MAGIC_COOKIE )
    {
      EwErrorS( 236, GetResourceName( aResource ));
      EwPanic();
      return 0;
    }
  }
  ...
  if ( !aResource->Compressed && 
     ( aResource->Format == EW_DRIVER_VARIANT_INDEX8 ))
  {
    const unsigned int* clut = (const unsigned int*)RES_PTR( Clut );

    /* If the memory of constant surfaces is not accessible by the CPU, use a
       'Reader' to load and map the clut pointer.  */
    #ifdef EW_USE_READER_FOR_CONST_SURFACES
      clut = (const unsigned int*)EwFlashAreaReaderProc( clut, 0 );
    #endif

    if ( clut[0] != EW_COLOR_CHANNELS_MAGIC_COOKIE )
    {
      EwErrorS( 237, GetResourceName( aResource ));
      EwPanic();
      return 0;
    }
  }
  ...
  if ( !aResource->Compressed && 
     ( aResource->Format == EW_DRIVER_VARIANT_RGB565 ))
  {
    const unsigned short* pixel = (const unsigned short*)RES_PTR( Pixel1 );

    /* If the memory of constant surfaces is not accessible by the CPU, use a
       'Reader' to load and map the pixel pointers.  */
    #ifdef EW_USE_READER_FOR_CONST_SURFACES
      pixel = (const unsigned short*)EwFlashAreaReaderProc( pixel, 0 );
    #endif

    if ( pixel[0] != EW_RGB565_COLOR_CHANNELS_MAGIC_COOKIE )
    {
      EwErrorS( 238, GetResourceName( aResource ));
      EwPanic();
      return 0;
    }
  }
...
*
*******************************************************************************/
const void* EwBmpGetCacheSearchTag( const struct XBmpRes* aResource )
{
  const XBmpRes* res     = (const XBmpRes*)aResource;
  #ifdef EW_USE_BLOB_DATA
    int          isBlob  = IS_BLOB_RES;
  #endif

  /* Each resource manages its own array with frames. Pointer to this array is
     used to identify the resource uniquely. In case of BLOB resources, the
     pointer addressing in the BLOB memory area. */
  return (const XBmpFrameRes*)RES_PTR( Frames );
}
...
*
*******************************************************************************/
int EwBmpGetMetrics( void* aHandle, int* aFormat, int* aNoOfFrames,
  int* aNoOfVirtFrames, int* aFrameWidth, int* aFrameHeight, int* aFrameDelay )
{
  const XBmpRes*        res     = (const XBmpRes*)aHandle;
    ...
    int                 isBlob  = IS_BLOB_RES;
  #endif
  const unsigned short* mapping = (const unsigned short*)RES_PTR( Mapping );
  int                   count   = 0;
  ...
  for ( ;; count++ )
  {
    const unsigned short* tmp = mapping + count;

    /* Not directly accessible memory area? Map the content in CPU address
       space. */
    if ( USE_FLASH_READER( tmp ))
      tmp = (const unsigned short*)EwFlashAreaReaderProc( tmp, 0 );

    /* End of mapping list terminator */
    if ( *tmp == 0xFFFF )
      break;
  }
...
*
*******************************************************************************/
int EwBmpGetFrameMetrics( void* aHandle, int aFrameNo, int* aOpqX,
  int* aOpqY, int* aOpqWidth, int* aOpqHeight )
{
  const XBmpRes*      res    = (const XBmpRes*)aHandle;
    ...
    int               isBlob = IS_BLOB_RES;
  #endif
  const XBmpFrameRes* frame  = (const XBmpFrameRes*)RES_PTR( Frames );
  ...
  /* Not directly accessible memory area? Map the content in CPU address
     space. */
  if ( USE_FLASH_READER( frame ))
    frame = (const XBmpFrameRes*)EwFlashAreaReaderProc( frame, sizeof( *frame ));
...
*
*******************************************************************************/
int EwBmpGetFrameMemory( void* aHandle, int aFrameNo, 
  XSurfaceMemory* aMemory )
{
  ...
  #endif
  const XBmpFrameRes*  frame  = (const XBmpFrameRes*)RES_PTR( Frames );
  const unsigned char* pixel  = (const unsigned char*)RES_PTR( Pixel1 );
  ...
  else if ( res->Format == EW_DRIVER_VARIANT_INDEX8 )
  {
    ...
    aMemory->Pitch2Y = 0;
    aMemory->Pitch2X = 0;
    aMemory->Clut    = (unsigned int*)RES_PTR( Clut ) + 1;
  }
...
*
*******************************************************************************/
int EwBmpLoadFrame( void* aHandle, int aFrameNo,
  XSurfaceMemory* aMemory )
{
  const XBmpRes*       res    = (const XBmpRes*)aHandle;
  #ifdef EW_USE_BLOB_DATA
    ...
    if ( res->Format == EW_DRIVER_VARIANT_RGB565 )
    {
      ...
      for ( y = 0; y < height; y++, dest += ofs )
        for ( x = 0; x < width; x++, dest++ )
        {
          ...
          c0 = EW_SHIFT( c0, 0  - EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_BLUE  );
          c1 = EW_SHIFT( c1, 5  - EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_GREEN );
          c2 = EW_SHIFT( c2, 11 - EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_RED   );
      ...
      for ( y = 0; y < height; y++, dest += ofs )
        for ( x = 0; x < width; x++, dest++ )
        {
          ...
          /* Change the color channel order */
            ...
            c0 = EW_SHIFT( c0, 0  - EW_COLOR_CHANNEL_BIT_OFFSET_BLUE  );
            c1 = EW_SHIFT( c1, 8  - EW_COLOR_CHANNEL_BIT_OFFSET_GREEN );
            c2 = EW_SHIFT( c2, 16 - EW_COLOR_CHANNEL_BIT_OFFSET_RED   );
...
*
*******************************************************************************/
int EwBmpLoadClut( void* aHandle, XSurfaceMemory* aMemory )
{
  const XBmpRes*      res    = (const XBmpRes*)aHandle;
  #ifdef EW_USE_BLOB_DATA
    int               isBlob = IS_BLOB_RES;
  ...
  /* Can't address the CLUT resource data directly. Therefore map the CLUT
     content in addressable CPU address space. */
  if ( !USE_FLASH_READER( clut ))
    EwCopy( aMemory->Clut, clut + 1, clut[0] * sizeof( int ));

  /* ... otherwise map the pointer to the clut entry before copying it */
  else
  {
    int count = *(const int*)EwFlashAreaReaderProc( clut, 0 );
    int i;

    /* The entries are copied individually */
    for ( i = 0, clut++; i < count; i++ )
      aMemory->Clut[i] = *(const unsigned int*)EwFlashAreaReaderProc( clut + i, 0 );
  }
...
*
*******************************************************************************/
int EwBmpLoadMappingTable( void* aHandle, unsigned short* aMapping )
{
  const XBmpRes*        res     = (const XBmpRes*)aHandle;
  #ifdef EW_USE_BLOB_DATA
    int                 isBlob  = IS_BLOB_RES;
