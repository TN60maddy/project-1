...
/* Helper macros for accessing the attribute information associated to entities
   ...
   32-bit DWORD value. */
#define GET_ATTR_TYPE( aA )   ((int)((aA)  & 0x0000000F ))
  ...
  XInt32 aMaxNoOfRows, XHandle aBidi )
{
  ...
  for ( srcPtr = srcStart; ( ch = *srcPtr ) != 0; )
  {
    ...
    if ( ch != '\n' )
    {
      ...
      /* The shaping engine has reported a possible line break candidate after
         the preceding cluster (and before this next cluster). Remember the
         location. */
      if ( wrapPending && width && !wrapActive )
      {
        /* Discard the last soft-hyphen wrap candidated */
        if ( wrapDstPtr && ( *wrapDstPtr == 0x00AD ))
          *wrapDstPtr = 0xFEFF;
      ...
      /* Clear the line break candidate reported by the shaping engine */
      wrapPending = 0;
      ...
      {
        int          shWidth = 0;
        unsigned int glyphId = EwGetGlyphId( aFont, ch );

        /* If applied - the soft-hyphen sign should be displayed ... Get the size
           of the soft-hyphen sign ... */
        if ( glyphId && EwGetGlyphMetrics( aFont, glyphId, &glyph ))
        {
          ...
          {
            if ( level & 1 )
              shWidth += EwGetKerning( aFont, glyphId, prevGlyphId );
            else
              shWidth += EwGetKerning( aFont, prevGlyphId, glyphId );
          }
        }
      ...
      if ( ch != 0xFEFF )
      {
        ...
        int          level        = 0;
        int          maxNoOfChars = (int)MIN( strLen - ( srcPtr - srcStart ), 8 );
        int          prevLevel2   = prevLevel;
        unsigned int prevGlyphId2 = prevGlyphId;
        int          i;
        ...
        if ( aBidi )
        {
          level = EwBidiGetCharLevel( aBidi, (int)( srcPtr - srcStart ));

          /* Determine the sequence of characters belonging to the same level. */
          for ( i = 1; i < maxNoOfChars; i++ )
            if ( level != EwBidiGetCharLevel( aBidi, (int)( srcPtr - srcStart ) + i ))
               maxNoOfChars = i;
        }

        /* Shape the next text cluster. */
        EwFntSaveShaper( context );
        noOfChars = EwFntShapeCluster( aFont->Handle, srcPtr, maxNoOfChars, 
                                       level & 1, glyphIds, deltasDummy,
                                       deltasDummy, deltasAdvance, &noOfGlyphIds,
                                       &lineEnd, context );
        ...
        for ( i = 0; i < noOfGlyphIds; i++ )
        {
          /* Get the metric of the glyph in question. If the operation fails,
             ignore the glyph */
          if ( EwGetGlyphMetrics( aFont, glyphIds[i], &glyph ))
          {
            chWidth += glyph.Advance + deltasAdvance[i];
            ...
            {
              if ( level & 1 )
                chWidth += EwGetKerning( aFont, glyphIds[i], prevGlyphId );
              else
                chWidth += EwGetKerning( aFont, prevGlyphId, glyphIds[i]);
            }
        ...
        {
          int noOfGlyphIds2 = sizeof( glyphIds ) / sizeof( glyphIds[0]);
          int chWidth2      = 0;
          int dummyLineEnd;
          ...
          /* Shape the next text cluster. Please note: do this relative to
             ...
             state. */
          EwFntRestoreShaper( context );
          EwFntShapeCluster( aFont->Handle, srcPtr, noOfChars, level & 1, 
                             glyphIds, deltasDummy, deltasDummy, deltasAdvance,
                             &noOfGlyphIds2, &dummyLineEnd, context );
          EwFntRestoreShaper( context );

          /* Process the glyphs. Calculate the advance sum for all glyphs. */
          for ( i = 0; i < noOfGlyphIds2; i++ )
          {
            /* Get the metric of the glyph in question. If the operation fails,
               ignore the glyph */
            if ( EwGetGlyphMetrics( aFont, glyphIds[i], &glyph ))
            {
              chWidth2 += glyph.Advance + deltasAdvance[i];
              ...
              {
                if ( level & 1 )
                  chWidth2 += EwGetKerning( aFont, glyphIds[i], prevGlyphId2 );
                else
                  chWidth2 += EwGetKerning( aFont, prevGlyphId2, glyphIds[i]);
              }
            
              /* Track the preceding glyph */
              prevGlyphId2 = glyphIds[i];
              prevLevel2   = level;
            }
          }
      ...
      if ((( chWidthMax + width ) <= maxWidth ) || !width )
      {
        ...
        /* According to the shaping engine there is a possible line break
           candidate after the current cluster. Remember it. */
        if ( lineEnd )
          wrapPending = 1;
      }
      ...
      /* No word wrap candidate found - force a wrap before the current 
         cluster */
      else
      {
        srcPtr = curSrcPtr;
  ...
  *aLeading  = 0;
  ...
  for ( ; *aChars; aChars++, aAttrs++ )
  {
        ...
        ( type == TYPE_IMAGE_TOP ))
    {
      ...
      /* Consume the entity representing the image */
      noOfChars = 1;
    ...
    else
    {
      ...
      if (( code == ' ' ) || ( code == 0x200B ) || wrapPending )
      {
      ...
      if ( code == 0x00AD )
      {
        unsigned int glyphId = font? EwGetGlyphId( font, code ) : 0;

        /* If applied - the '-' minus sign should be displayed ... Get the size
           of the '-' minus sign ... */
        if ( glyphId && *aWidth && EwGetGlyphMetrics( font, glyphId, &glyph ))
        {
          ...
          {
            if ( level & 1 )
              curWidth += EwGetKerning( font, glyphId, prevGlyphId );
            else
              curWidth += EwGetKerning( font, prevGlyphId, glyphId );
          }
        }
      ...
      if ( font )
      {
        ...
        if ( aBidi )
        {
          level = EwBidiGetCharLevel( aBidi, aBidiIndex + (int)( aChars - 
                                      startChars ));

          /* Determine the sequence of characters belonging to the same level. */
          for ( i = 1; i < maxNoOfChars; i++ )
            if ( level != EwBidiGetCharLevel( aBidi, aBidiIndex + (int)
                                            ( aChars - startChars ) + i ))
               maxNoOfChars = i;
        }
        ...
        /* Shape the next text cluster. */
        EwFntSaveShaper( context );
        noOfChars = EwFntShapeCluster( font->Handle, aChars, maxNoOfChars, 
                                       level & 1, glyphIds, deltasDummy,
                                       deltasDummy, deltasAdvance,
                                       &noOfGlyphIds, &lineEnd, context );
        ...
        for ( i = 0; i < noOfGlyphIds; i++ )
        {
          /* Get the metric of the glyph in question. If the operation fails,
             ignore the glyph */
          if ( EwGetGlyphMetrics( font, glyphIds[i], &glyph ))
          {
            chWidth += glyph.Advance + deltasAdvance[i];
            ...
            {
              if ( level & 1 )
                chWidth += EwGetKerning( font, glyphIds[i], prevGlyphId );
              else
                chWidth += EwGetKerning( font, prevGlyphId, glyphIds[i]);
            }
           ...
           (( aMaxWidth -  *aWidth ) < ( 3 * chWidthMax )))
        {
          ...
          /* Shape the next text cluster. Please note: do this relative to
             ...
             state. */
          EwFntRestoreShaper( context );
          EwFntShapeCluster( font->Handle, aChars, noOfChars, level & 1, 
                             glyphIds, deltasDummy, deltasDummy,
                             deltasAdvance, &noOfGlyphIds2, &dummyLineEnd,
                             context );
          EwFntRestoreShaper( context );
          ...
          for ( i = 0; i < noOfGlyphIds2; i++ )
          {
            ...
            if ( EwGetGlyphMetrics( font, glyphIds[i], &glyph ))
            {
              ...
              {
                if ( level & 1 )
                  chWidth2 += EwGetKerning( font, glyphIds[i], prevGlyphId2 );
                else
                  chWidth2 += EwGetKerning( font, prevGlyphId2, glyphIds[i]);
              }
      ...
      /* No font to display the character. Skip over it. */
      else
        noOfChars++;
    }
    ...
    if ((( curWidthMax + *aWidth ) <= aMaxWidth ) || !*aWidth )
    {
      *aWidth   += curWidth;
      ...
      *aSpace   += curSpace;
      ...
      /* Shaping engine has reported a potential line break opportunity just
         behind the actual cluster. Remember it. */
      if ( lineEnd )
        wrapPending = 1;
  ...
  while ( inx < len )
  {
    ...
    for ( ; count >= 0; count--, inx++ )
    {
         ...
         ( ulOn != newUlOn ) || ( linkId != newLinkId )))
      {
        ...
        if ( ulOn )
        {
          unsigned int glyphId = font? EwGetGlyphId( font, '-' ) : 0;

          if ( glyphId && EwGetGlyphMetrics( font, glyphId, &glyph ))
            ulOn = glyph.Size.Y;
           ...
           ( code != 0xFEFF ))
      {
        ...
        /* Shape the next text cluster. */
        noOfChars = EwFntShapeCluster( font->Handle, chars + inx, maxNoOfChars, 
                                       0, glyphIds, deltasDummy, deltasDummy,
                                       deltasAdvance, &noOfGlyphIds, &dummyLineEnd,
                                       context );
        ...
        for ( i = 0; i < noOfGlyphIds; i++ )
        {
          ...
          if ( EwGetGlyphMetrics( font, glyphIds[i], &glyph ))
          {
            ...
            if ( first )
            {
              kern  = EwGetKerning( font, prevGlyphId, glyphIds[0]);
              first = 0;
            }
            else
              chWidth += EwGetKerning( font, prevGlyphId, glyphIds[i]);
