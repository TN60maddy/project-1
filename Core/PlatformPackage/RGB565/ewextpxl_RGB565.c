...
/* Calculate the luminance for a given Native color value */
#define LUMINANCE_NATIVE( aColor )                                             \
  (((((( aColor ) >> EW_COLOR_CHANNEL_BIT_OFFSET_RED   ) & 0xFF ) *  77 ) +    \
    (((( aColor ) >> EW_COLOR_CHANNEL_BIT_OFFSET_GREEN ) & 0xFF ) * 150 ) +    \
    (((( aColor ) >> EW_COLOR_CHANNEL_BIT_OFFSET_BLUE  ) & 0xFF ) *  29 )) >> 8 )
...
/* If the memory of constant surfaces is not accessible by the CPU, use a
  ...
  #define MAP_UINT8( aPtr )                                                    \
   (( aSrc->Reader )? (unsigned char*)aSrc->Reader( aPtr, 0 ) : ( aPtr ))
  #define MAP_UINT16( aPtr )                                                   \
   (( aSrc->Reader )? (unsigned short*)aSrc->Reader( aPtr, 0 ) : ( aPtr ))
  #define MAP_UINT32( aPtr )                                                   \
   (( aSrc->Reader )? (unsigned int*)aSrc->Reader( aPtr, 0 ) : ( aPtr ))
#else
...
/* The following macro provides a code fragment for the modulation of a native 
   color value aColor by the opacity of the source value aSrc. The result is
   then mixed with aSrc value according to the factor aIntensity. aDst refers
   to destination variable, where the operation result should be left. */
#ifdef EW_PREMULTIPLY_COLOR_CHANNELS
  #define TINT_NATIVE_OPACITY( aDst, aColor, aSrc, aIntensity )                \
    {                                                                          \
      /* Bring all color channels into 2 x 32-bit register and ... */          \
      unsigned int _alpha  = CHANNEL_ALPHA( aSrc ) + 1;                        \
      unsigned int _clrC13 =   aColor        & mask2;                          \
      unsigned int _clrC24 = ( aColor >> 8 ) & mask2;                          \
                                                                               \
      /* Modulate the color by opacity */                                      \
      if ( _alpha < 256 )                                                      \
      {                                                                        \
        _clrC13 = (( _clrC13 * _alpha ) >> 8 ) & mask2;                        \
        _clrC24 = (( _clrC24 * _alpha ) >> 8 ) & mask2;                        \
      }                                                                        \
                                                                               \
      /* Interpolate between the calculated and the original colors */         \
      if ( aIntensity < 256 )                                                  \
      {                                                                        \
        /* Bring all color channels into 2 x 32-bit register and ... */        \
        unsigned int _srcC13 =   aSrc        & mask2;                          \
        unsigned int _srcC24 = ( aSrc >> 8 ) & mask2;                          \
        unsigned int _mix2   = 256 - aIntensity;                               \
                                                                               \
        /* Interpolate between the both colors */                              \
        _clrC13 = ( _clrC13 * aIntensity ) + ( _srcC13 * _mix2 );              \
        _clrC24 = ( _clrC24 * aIntensity ) + ( _srcC24 * _mix2 );              \
        _clrC13 = ( _clrC13 >> 8 ) & mask2;                                    \
        _clrC24 = ( _clrC24 >> 8 ) & mask2;                                    \
      }                                                                        \
                                                                               \
      /* Compose the results */                                                \
      aDst = _clrC13 | ( _clrC24 << 8 );                                       \
    }
#else
  #define TINT_NATIVE_OPACITY( aDst, aColor, aSrc, aIntensity )                \
    {                                                                          \
      unsigned int _alpha  = CHANNEL_ALPHA( aSrc ) + 1;                        \
      unsigned int _clr    = aColor;                                           \
      unsigned int _clrA   = ( CHANNEL_ALPHA( _clr ) * _alpha ) >> 8;          \
                                                                               \
      /* Modulate the opacity of the color */                                  \
      _clr &= ~( 0xFFu << EW_COLOR_CHANNEL_BIT_OFFSET_ALPHA );                 \
      _clr |=  ( _clrA << EW_COLOR_CHANNEL_BIT_OFFSET_ALPHA );                 \
                                                                               \
      /* Interpolate between the calculated and the original colors */         \
      if ( aIntensity < 256 )                                                  \
      {                                                                        \
        /* Bring all color channels into 2 x 32-bit register and ... */        \
        unsigned int _clrC13 =   _clr         & mask2;                         \
        unsigned int _clrC24 = ( _clr  >> 8 ) & mask2;                         \
        unsigned int _srcC13 =   aSrc         & mask2;                         \
        unsigned int _srcC24 = ( aSrc >> 8 ) & mask2;                          \
        unsigned int _mix2    = 256 - aIntensity;                              \
                                                                               \
        /* Interpolate between the calculated and original colors */           \
        _clrC13 = ( _clrC13 * aIntensity ) + ( _srcC13 * _mix2 );              \
        _clrC24 = ( _clrC24 * aIntensity ) + ( _srcC24 * _mix2 );              \
        _clr    = (( _clrC13 >> 8 ) & mask2 ) | ( _clrC24 & ( mask2 << 8 ));   \
      }                                                                        \
                                                                               \
      /* Compose the results */                                                \
      aDst = _clr;                                                             \
...
/* The following macro provides a code fragment for the modulation of the RGB
   chanels of the native color value aColor by the luminance of the source
   value aSrc. The result is then mixed with aSrc value according to the factor
   aIntensity. aDst refers to destination variable, where the operation result
   should be left. */
#ifdef EW_PREMULTIPLY_COLOR_CHANNELS
  #define TINT_NATIVE_LUMINANCE( aDst, aColor, aSrc, aIntensity )              \
    {                                                                          \
      unsigned int _lum = LUMINANCE_NATIVE( aSrc ) + 1;                        \
      unsigned int _clr = aColor;                                              \
                                                                               \
      /* Modulate the color by luminance */                                    \
      if ( _lum < 256 )                                                        \
      {                                                                        \
        unsigned int _alpha  = CHANNEL_ALPHA( _clr );                          \
        unsigned int _clrC13 =   _clr        & mask2;                          \
        unsigned int _clrC24 = ( _clr >> 8 ) & mask2;                          \
        _clrC13 = (( _clrC13 * _lum ) >> 8 ) & mask2;                          \
        _clrC24 =  ( _clrC24 * _lum ) & ( mask2 << 8 );                        \
        _alpha  = ( _alpha * ( CHANNEL_ALPHA( aSrc ) + 1 )) >> 8;              \
        _clr    = _clrC13 | _clrC24;                                           \
        _clr   &= ~( 0xFFu << EW_COLOR_CHANNEL_BIT_OFFSET_ALPHA );             \
        _clr   |=  _alpha  << EW_COLOR_CHANNEL_BIT_OFFSET_ALPHA;               \
      }                                                                        \
                                                                               \
      /* Interpolate between the calculated and the original colors */         \
      if ( aIntensity < 256 )                                                  \
...
/* The following macro provides a code fragment for the modulation of a native 
   color value aColor by the opacity aSrc. The result is then mixed with a white
   color value according to the factor aIntensity. aDst refers to destination
   variable, where the operation result should be left. */
#ifdef EW_PREMULTIPLY_COLOR_CHANNELS
  #define TINT_ALPHA8_OPACITY( aDst, aColor, aSrc, aIntensity )                \
    {                                                                          \
      /* Bring all color channels into 2 x 32-bit register and ... */          \
      unsigned int _alpha  =   aSrc + 1;                                       \
      unsigned int _clrC13 =   aColor        & mask2;                          \
      unsigned int _clrC24 = ( aColor >> 8 ) & mask2;                          \
                                                                               \
      /* Modulate the color by opacity */                                      \
      ...
      {                                                                        \
        ...
        _clrC24 = (( _clrC24 * _alpha ) >> 8 ) & mask2;                        \
                                                                               ...
                                                                               \
      /* Interpolate between the calculated and the original colors.           \
         Note: in ALPHA8 the color is white. */                                \
      if ( aIntensity < 256 )                                                  \
      {                                                                        \
        unsigned int _srcC = (( mask2 * _alpha ) >> 8 ) & mask2;               \
                                                                               \
        _srcC *= 256 - aIntensity;                                             \
                                                                               \
        /* Interpolate between the both colors */                              \
        _clrC13 = ( _clrC13 * aIntensity ) + _srcC;                            \
        _clrC24 = ( _clrC24 * aIntensity ) + _srcC;                            \
        _clrC13 = ( _clrC13 >> 8 ) & mask2;                                    \
        _clrC24 = ( _clrC24 >> 8 ) & mask2;                                    \
      }                                                                        \
                                                                               \
  ...
  #define TINT_ALPHA8_OPACITY( aDst, aColor, aSrc, aIntensity )                \
    {                                                                          \
      ...
      if ( aIntensity < 256 )                                                  \
      {                                                                        \
        unsigned int _src = ( 0xFFFFFFFF ^                                     \
                            ( 0xFFu << EW_COLOR_CHANNEL_BIT_OFFSET_ALPHA )) |  \
                            ( aSrc  << EW_COLOR_CHANNEL_BIT_OFFSET_ALPHA );    \
                                                                               \
        /* Bring all color channels into 2 x 32-bit register and ... */        \
        unsigned int _clrC13 =   _clr        & mask2;                          \
        unsigned int _clrC24 = ( _clr >> 8 ) & mask2;                          \
...
/* The following macro provides a code fragment for the modulation of the RGB
   ...
   should be left. */
#ifdef EW_PREMULTIPLY_COLOR_CHANNELS
  #define TINT_RGB565_LUMINANCE( aDst, aColor, aSrc, aIntensity )              \
    {                                                                          \
      /* Convert RGB565 in the Native format */                                \
      RGB565_2_NATIVE_OPAQUE( aSrc, aSrc );                                    \
                                                                               \
...
/* Following macro takes care of pre-multiplying the RGB channels of a NATIVE
   color by the corresponding ALPHA channel. */
#ifndef EW_PREMULTIPLY_COLOR_CHANNELS
  #if EW_COLOR_CHANNEL_BIT_OFFSET_ALPHA == 24
    #define PREMULTIPLY_NATIVE_CHANNELS( aC1, aC2, aC3, aC4 )                  \
    {                                                                          \
      if ( aC4 < 255 )                                                         \
      {                                                                        \
        unsigned int _alpha = aC4 + 1;                                         \
        aC1 = ( aC1 * _alpha ) >> 8;                                           \
        aC2 = ( aC2 * _alpha ) >> 8;                                           \
        aC3 = ( aC3 * _alpha ) >> 8;                                           \
      }                                                                        \
  ...
  #if EW_COLOR_CHANNEL_BIT_OFFSET_ALPHA == 16
    #define PREMULTIPLY_NATIVE_CHANNELS( aC1, aC2, aC3, aC4 )                  \
    {                                                                          \
      if ( aC3 < 255 )                                                         \
      {                                                                        \
        unsigned int _alpha = aC3 + 1;                                         \
        aC1 = ( aC1 * _alpha ) >> 8;                                           \
        aC2 = ( aC2 * _alpha ) >> 8;                                           \
        aC4 = ( aC4 * _alpha ) >> 8;                                           \
      }                                                                        \
  ...
  #if EW_COLOR_CHANNEL_BIT_OFFSET_ALPHA == 8
    #define PREMULTIPLY_NATIVE_CHANNELS( aC1, aC2, aC3, aC4 )                  \
    {                                                                          \
      if ( aC2 < 255 )                                                         \
      {                                                                        \
        unsigned int _alpha = aC2 + 1;                                         \
        aC1 = ( aC1 * _alpha ) >> 8;                                           \
        aC3 = ( aC3 * _alpha ) >> 8;                                           \
        aC4 = ( aC4 * _alpha ) >> 8;                                           \
      }                                                                        \
  ...
  /* The color channels are alpha-premultiplied per default. Nothing to do. */
#else
  #define PREMULTIPLY_NATIVE_CHANNELS( t1, t2, t3, t4 )
#endif


/* The following macro provides a code fragment to copy native color value. The
   expressions aSrc and aDst result in the native color value and the variable,
   where the operation result should be left. Please note, this version assumes
   aSrc contains a NATIVE color with its RGB channels being already alpha-pre-
   multiplied - even on a target system working without the premultiplication. */
#ifdef EW_PREMULTIPLY_COLOR_CHANNELS
  #define COPY_PREMULT_NATIVE( aDst, aSrc )                                    \
    {                                                                          \
      aDst = aSrc;                                                             \
    }
#else
  #define COPY_PREMULT_NATIVE( aDst, aSrc )                                    \
    {                                                                          \
      unsigned int _srcA = CHANNEL_ALPHA( aSrc );                              \
                                                                               \
      /* Is it necessary to modulate the color channels? */                    \
      if ( _srcA < 255 )                                                       \
      {                                                                        \
        unsigned int  _src    =    aSrc ^                                      \
                                ( _srcA << EW_COLOR_CHANNEL_BIT_OFFSET_ALPHA );\
        unsigned int  _srcC13 =   _src        & mask2;                         \
        unsigned int  _srcC24 = ( _src >> 8 ) & mask2;                         \
        unsigned int  _inv    = InvAlpha[ _srcA ];                             \
                                                                               \
        _srcC13 = (( _srcC13 & mask2 ) * _inv ) >> 8;                          \
        _srcC24 = (( _srcC24 & mask2 ) * _inv ) >> 8;                          \
                                                                               \
        aSrc = ( _srcC13 & mask2 ) | (( _srcC24 & mask2 ) << 8 ) |             \
               ( _srcA << EW_COLOR_CHANNEL_BIT_OFFSET_ALPHA );                 \
      }                                                                        \
                                                                               \
      aDst = aSrc;                                                             \
...
/* The following macro provides a code fragment for the modulation of a native 
   color value by an opacity value. The expressions aSrc and aOpacity result in
   the native color value and the opacity value to modulate it. aDst refers to
   destination variable, where the operation result should be left. Please note,
   this version assumes aSrc contains a NATIVE color with its RGB channels being
   already alpha-premultiplied - even on a target system working without the
   premultiplication. */
#ifdef EW_PREMULTIPLY_COLOR_CHANNELS
  #define COPY_PREMULT_NATIVE_OPACITY( aDst, aSrc, aOpacity )                  \
    {                                                                          \
      /* Is it necessary to modulate the color channels? */                    \
      if ( aSrc && ( aOpacity < 256 ))                                         \
      {                                                                        \
        /* Bring all color channels into 2 x 32-bit register and ... */        \
        unsigned int _srcC13 =   aSrc        & mask2;                          \
        unsigned int _srcC24 = ( aSrc >> 8 ) & mask2;                          \
  ...
  #define COPY_PREMULT_NATIVE_OPACITY( aDst, aSrc, aOpacity )                  \
    {                                                                          \
      unsigned int _srcA = CHANNEL_ALPHA( aSrc );                              \
                                                                               \
      /* Is it necessary to modulate the color channels? */                    \
      if (( aOpacity + _srcA ) < ( 256 + 255 ))                                \
      {                                                                        \
        unsigned int  _src    =    aSrc ^                                      \
                                ( _srcA << EW_COLOR_CHANNEL_BIT_OFFSET_ALPHA );\
        unsigned int  _srcC13 =   _src        & mask2;                         \
        unsigned int  _srcC24 = ( _src >> 8 ) & mask2;                         \
        unsigned int  _inv    = InvAlpha[ _srcA = ( aOpacity * _srcA ) >> 8 ]; \
                                                                               \
        _srcC13 = ( _srcC13 * aOpacity ) >> 8;                                 \
        _srcC24 = ( _srcC24 * aOpacity ) >> 8;                                 \
                                                                               \
        _srcC13 = (( _srcC13 & mask2 ) * _inv ) >> 8;                          \
        _srcC24 = (( _srcC24 & mask2 ) * _inv ) >> 8;                          \
                                                                               \
...
/* The following macro provides a code fragment for alpha-blending calculation
   ...
   The results of the alpha-blending are left in aDst. */
#ifdef EW_PREMULTIPLY_COLOR_CHANNELS
  #define BLEND_PREMULT_NATIVE_OPACITY( aDst, aSrc, aOpacity )                 \
    BLEND_NATIVE_OPACITY( aDst, aSrc, aOpacity )
#else
  #define BLEND_PREMULT_NATIVE_OPACITY( aDst, aSrc, aOpacity )                 \
    {                                                                          \
      unsigned int _srcA = CHANNEL_ALPHA( aSrc );                              \
                                                                               \
      /* Avoid blending if the pixel is transparent */                         \
      if ( _srcA )                                                             \
      {                                                                        \
        /* Extract the alpha-channel */                                        \
        unsigned int  _dstA   = CHANNEL_ALPHA( aDst );                         \
...
/* The following macro provides a code fragment for the modulation of a native 
   ...
   already alpha-premultiplied - even on a target system working without the
   premultiplication. */
#define SCREEN_COPY_PREMULT_NATIVE_OPACITY( aDst, aSrc, aOpacity )             \
  {                                                                            \
    unsigned int _src = aSrc;                                                  \
                                                                               \
    /* Anything to do? */                                                      \
    ...
    {                                                                          \
      ...
      {                                                                        \
        ...
        _srcC24 *= aOpacity;                                                   \
                                                                               \
        /* Compose the results */                                              \
        _src = (( _srcC13 >> 8 ) & mask2 ) | ( _srcC24 & ( mask2 << 8 ));      \
      }                                                                        \
                                                                               \
      /* Convert Native to RGB565 */                                           \
      _src = NATIVE_2_SCREEN_RED ( _src ) | NATIVE_2_SCREEN_GREEN( _src ) |    \
             NATIVE_2_SCREEN_BLUE( _src );                                     \
    }                                                                          \
                                                                               \
    /* Store the results */                                                    \
...
/* The following macro provides a code fragment for alpha-blending calculation
   between two native color values with an additional opacity factor. The color
   values and the opacity are passed in the aDst, aSrc and aOpacity expressions.
   The results of the alpha-blending are left in aDst. Please note, this version
   assumes aSrc contains a NATIVE color with its RGB channels being already 
   alpha-premultiplied - even on a target system working without the
   premultiplication.*/
#define SCREEN_BLEND_PREMULT_NATIVE_OPACITY( aDst, aSrc, aOpacity )            \
  {                                                                            \
    unsigned int _srcA = CHANNEL_ALPHA( src );                                 \
                                                                               \
    /* Anything to do? */                                                      \
    if ( _srcA )                                                               \
    {                                                                          \
      /* Alpha-blending / interpolation necessary? */                          \
      if (( _srcA + aOpacity ) < ( 256 + 255 ))                                \
      {                                                                        \
        /* Bring all color channels into 2 x 32-bit register and ... */        \
        unsigned int _srcC13 =   aSrc        & mask2;                          \
        unsigned int _srcC24 = ( aSrc >> 8 ) & mask2;                          \
        unsigned int _dst0   = *dstPtr;                                        \
        unsigned int _dstR   = GET_RGB565_RED  ( _dst0 );                      \
        unsigned int _dstG   = GET_RGB565_GREEN( _dst0 );                      \
        unsigned int _dstB   = GET_RGB565_BLUE ( _dst0 );                      \
        unsigned int _dst    = (( _dstR | ( _dstR >> 5 )) <<                   \
        ...
        _srcC13 &= mask2;                                                      \
        _srcC24 &= mask2;                                                      \
        aSrc     = _srcC13 | ( _srcC24 << 8 );                                 \
      }                                                                        \
                                                                               \
      /* Compose the results */                                                \
      *dstPtr = (unsigned short)( NATIVE_2_SCREEN_RED  ( aSrc ) |              \
                                  NATIVE_2_SCREEN_GREEN( aSrc ) |              \
                                  NATIVE_2_SCREEN_BLUE ( aSrc ));              \
    }                                                                          \
  }
...
/* The following macro provides a code fragment for linear color interpolation
   between two 32-bit packed color values according to the factor. The color
   values and the factor are passed in the aSrc1, aSrc2 and aFactor expressions.
   The results of the interpolation is left in aDst. */
#define MIX_PACKED_COLORS( aDst, aSrc1, aSrc2, aFactor )                       \
  {                                                                            \
    unsigned int  _nf      = 0x100 - aFactor;                                  \
    unsigned int  _src1    = aSrc1;                                            \
    unsigned int  _src2    = aSrc2;                                            \
    unsigned int  _src1C13 =   _src1        & mask2;                           \
    unsigned int  _src1C24 = ( _src1 >> 8 ) & mask2;                           \
    unsigned int  _src2C13 =   _src2        & mask2;                           \
    unsigned int  _src2C24 = ( _src2 >> 8 ) & mask2;                           \
                                                                               \
    /* Interpolate between the colors */                                       \
    _src1C13 = ((( _src1C13 * _nf ) + ( _src2C13 * aFactor )) >> 8 );          \
    _src1C24 = ((( _src1C24 * _nf ) + ( _src2C24 * aFactor )) >> 8 );          \
                                                                               \
    /* Compose the results */                                                  \
    aDst = ( _src1C13 & mask2 ) | (( _src1C24 & mask2 ) << 8 );                \
  }


/* The following macro provides a code fragment to convert between the 32-bit
   packed color format and the target specific native color format. The packed
   color value is passed in aSrc. The resulting native color value is left in
   the operand aDst. */
  ...
  #define PACKED_COLOR_2_NATIVE( aDst, aSrc )                                  \
    {                                                                          \
      unsigned int red   = ( aSrc >>  0 ) & 0xFF;                              \
      unsigned int green = ( aSrc >>  8 ) & 0xFF;                              \
      unsigned int blue  = ( aSrc >> 16 ) & 0xFF;                              \
      unsigned int alpha = ( aSrc >> 24 ) & 0xFF;                              \
                                                                               \
      aDst = EW_SHIFT( red,   - EW_COLOR_CHANNEL_BIT_OFFSET_RED   ) |          \
             EW_SHIFT( green, - EW_COLOR_CHANNEL_BIT_OFFSET_GREEN ) |          \
             EW_SHIFT( blue,  - EW_COLOR_CHANNEL_BIT_OFFSET_BLUE  ) |          \
             EW_SHIFT( alpha, - EW_COLOR_CHANNEL_BIT_OFFSET_ALPHA );           \
    }
#else
  #define PACKED_COLOR_2_NATIVE( aDst, aSrc )                                  \
      aDst = aSrc
...
void* EwCreateNativeSurface( int aWidth, int aHeight )
{
...
void* EwCreateConstNativeSurface( int aWidth, int aHeight,
...
void EwDestroyNativeSurface( void* aSurface )
{
...
int EwGetNativeSurfaceMemory( void* aSurface, int aX, int aY, 
...
/* The following low-level driver function has the job to create and return a
   ...
   to memory deficit, 0 is returned. */
void* EwCreateScreenSurface( int aWidth, int aHeight )
{
...
void EwDestroyScreenSurface( void* aSurface )
{
...
/* The following low-level driver function has the job to obtain a pointer to
   ...
   If successful, the function returns != 0. */
int EwGetScreenSurfaceMemory( void* aSurface, int aX, int aY, 
  int aIndex, int aWriteAccess, XSurfaceMemory* aMemory )
{
  XScreenSurface* surface = (XScreenSurface*)aSurface;
...
*
*******************************************************************************/
void* EwCreateConstRGB565Surface( int aWidth, int aHeight,
  XSurfaceMemory* aMemory )
{
...
*
*******************************************************************************/
void* EwCreateRGB565Surface( int aWidth, int aHeight )
{
  int             size    = aWidth * aHeight * sizeof( short );
...
*
*******************************************************************************/
void EwDestroyRGB565Surface( void* aSurface )
{
...
*
*******************************************************************************/
int EwGetRGB565SurfaceMemory( void* aSurface, int aX, int aY, int aIndex,
...
unsigned int EwPackColor( int aRed, int aGreen, int aBlue, int aAlpha,
  int aPixelFormat )
{
  if ( aPixelFormat == EW_PIXEL_FORMAT_SCREEN )
  {
    aRed   = ( aRed   + 4 ) >> 3;
    aGreen = ( aGreen + 2 ) >> 2;
    aBlue  = ( aBlue  + 4 ) >> 3;
    ...
    if ( aRed   > 31 ) aRed   = 31;
    if ( aGreen > 63 ) aGreen = 63;
    if ( aBlue  > 31 ) aBlue  = 31;
    if ( aAlpha > 63 ) aAlpha = 63;

    #ifdef EW_PREMULTIPLY_COLOR_CHANNELS
      aRed   = (( aRed   * ( aAlpha + 1 )) + 31 ) >> 6;
      aGreen = (( aGreen * ( aAlpha + 1 )) + 31 ) >> 6;
      aBlue  = (( aBlue  * ( aAlpha + 1 )) + 31 ) >> 6;

      if ( aRed   > aAlpha ) aRed   = aAlpha;
      if ( aGreen > aAlpha ) aGreen = aAlpha;
      if ( aBlue  > aAlpha ) aBlue  = aAlpha;
    #endif

    aRed   = ( aRed   << 3 ) | ( aRed   >> 2 );
    aGreen = ( aGreen << 2 ) | ( aGreen >> 4 );
    aBlue  = ( aBlue  << 3 ) | ( aBlue  >> 2 );
    aAlpha = ( aAlpha << 2 ) | ( aAlpha >> 4 );
...
/* Fill a pixel row with the content of the source surface modulating the pixel
   by an additional opacity value. The pixel will be written in the destination
   memory or alpha-blended with the existing pixel in the destination memory. */
void EwFilterNative( XSurfaceMemory* aDst, XSurfaceMemory* aSrc, 
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int   mask2  = 0x00FF00FF;
  unsigned int*  dstPtr = aDst ->Pixel1;
  unsigned int*  srcPtr = aSrc ->Pixel1;
  int            dstOfs = aDst->Pitch1X >> 2;
  ...
  while ( aWidth-- > 0 )
  {
    ...
    /* Combine the resulting color with the destination pixel by alpha blending
       or overwrite the pixel in the destination */
    if ( aBlend ) BLEND_NATIVE_OPACITY( *dstPtr, src, aOpacity )
    else          COPY_NATIVE_OPACITY ( *dstPtr, src, aOpacity )
...
/* Fill a pixel row with the content of the source surface applying a mask
   filter with additional opacity value. The new pixel will be written in the
   destination memory or alpha-blended with the existing pixel in the destination
   memory. */
void EwFilterNativeMask( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int   mask2  = 0x00FF00FF;
  unsigned int*  dstPtr = aDst ->Pixel1;
  unsigned int*  srcPtr = aSrc ->Pixel1;
  unsigned char* mskPtr = aMask->Pixel1;
  int            dstOfs = aDst->Pitch1X >> 2;
  int            mskOfs = aMask->Pitch1X;
  ...
  {
    unsigned int src = *MAP_UINT32( srcPtr++ );
    unsigned int msk = *MAP_UINT8 ( mskPtr );

    /* Invert the effect of the mask */
    if ( aMaskInverted  )
      msk = 255 - msk;

    /* Modulate the mask with the opacity. The resulting value should lie in
       range 0 .. 256 */
    if ( msk )
      msk = ((( msk + 1 ) * aOpacity ) >> 8 ) + 1;

    /* Combine the resulting color with the destination pixel by alpha blending
       or overwrite the pixel in the destination */
    if ( aBlend ) BLEND_NATIVE_OPACITY( *dstPtr, src, msk )
    else          COPY_NATIVE_OPACITY ( *dstPtr, src, msk )
...
/* Fill a pixel row with the content of the source surface applying a tint by
   opacity filter with additional opacity value. The new pixel will be written
   in the destination memory or alpha-blended with the existing pixel in the
   destination memory. */
void EwFilterNativeTintByOpacity( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int   mask1  = 0x0FF00000;
  ...
  unsigned int*  srcPtr = aSrc ->Pixel1;
  int            dstOfs = aDst->Pitch1X >> 2;
  unsigned int   clr    = GRADIENT_TO_COLOR( aColor->R0, aColor->G0,
                                             aColor->B0, aColor->A0 );

  EW_UNUSED_ARG( aMask );
  EW_UNUSED_ARG( aMaskInverted );

  /* Stretch the values from range 0..255 to 0..256 to allow shift operations
     instead of divisions */
  aIntensity++;
  aOpacity++;
  ...
  while ( aWidth-- > 0 )
  {
    ...
    /* Modulate the color value by the opacity of the source pixel and mix
       the resulting color with the original color from the source */
    TINT_NATIVE_OPACITY( src, clr, src, aIntensity );
...
/* Fill a pixel row with the content of the source surface applying a tint by
   luminance filter with additional opacity value. The new pixel will be written
   in the destination memory or alpha-blended with the existing pixel in the
   destination memory. */
void EwFilterNativeTintByLuminance( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int   mask1  = 0x0FF00000;
  ...
  while ( aWidth-- > 0 )
  {
    ...
    /* Modulate the color value by the luminance of the source pixel and mix
       the resulting color with the original color from the source */
    TINT_NATIVE_LUMINANCE( src, clr, src, aIntensity );
...
/* Fill a pixel row with the content of the source surface applying a mask and
   tint by opacity filter with additional opacity value. The new pixel will be
   written in the destination memory or alpha-blended with the existing pixel in
   the destination memory. */
void EwFilterNativeMaskTintByOpacity( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int   mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a mask and
   tint by luminance filter with additional opacity value. The new pixel will be
   written in the destination memory or alpha-blended with the existing pixel in
   the destination memory. */
void EwFilterNativeMaskTintByLuminance( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int   mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a mask
   ...
   destination memory or alpha-blended with the existing pixel in the destination
   memory. */
void EwFilterIndex8Mask( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int   mask2  = 0x00FF00FF;
...
/* Fill a pixel row with the content of the source surface applying a tint by
   ...
   in the destination memory or alpha-blended with the existing pixel in the
   destination memory. */
void EwFilterIndex8TintByOpacity( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int   mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a tint by
   ...
   in the destination memory or alpha-blended with the existing pixel in the
   destination memory. */
void EwFilterIndex8TintByLuminance( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int   mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a mask and
   ...
   written in the destination memory or alpha-blended with the existing pixel in
   the destination memory. */
void EwFilterIndex8MaskTintByOpacity( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int   mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a mask and
   ...
   written in the destination memory or alpha-blended with the existing pixel in
   the destination memory. */
void EwFilterIndex8MaskTintByLuminance( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int   mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a mask
   ...
   destination memory or alpha-blended with the existing pixel in the destination
   memory. */
void EwFilterAlpha8Mask( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  /* ALPHA8 has no color information to mask. This fiter can be ignored. */
  EW_UNUSED_ARG( aDst          );
  EW_UNUSED_ARG( aSrc          );
  EW_UNUSED_ARG( aMask         );
...
/* Fill a pixel row with the content of the source surface applying a tint by
   ...
   in the destination memory or alpha-blended with the existing pixel in the
   destination memory. */
void EwFilterAlpha8TintByOpacity( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int   mask1  = 0x0FF00000;
  ...
  while ( aWidth-- > 0 )
  {
    ...
    /* Modulate the color value by the opacity of the source pixel and mix
       the resulting color with the original color from the source */
    TINT_ALPHA8_OPACITY( src, clr, src, aIntensity );
...
/* Fill a pixel row with the content of the source surface applying a tint by
   ...
   in the destination memory or alpha-blended with the existing pixel in the
   destination memory. */
void EwFilterAlpha8TintByLuminance( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  /* ALPHA8 has luminance == 1. This filter can be ignored. */
  EW_UNUSED_ARG( aDst          );
  EW_UNUSED_ARG( aSrc          );
  EW_UNUSED_ARG( aMask         );
  EW_UNUSED_ARG( aWidth        );
...
/* Fill a pixel row with the content of the source surface applying a mask and
   ...
   written in the destination memory or alpha-blended with the existing pixel in
   the destination memory. */
void EwFilterAlpha8MaskTintByOpacity( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int   mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a mask and
   ...
   written in the destination memory or alpha-blended with the existing pixel in
   the destination memory. */
void EwFilterAlpha8MaskTintByLuminance( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  /* ALPHA8 has luminance == 1. This filter can be ignored. */
...
/* Fill a pixel row with the content of the source surface applying a mask
   ...
   destination memory or alpha-blended with the existing pixel in the destination
   memory. */
void EwFilterRGB565Mask( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask2  = 0x00FF00FF;
  ...
  while ( aWidth-- > 0 )
  {
    ...
    /* Modulate the mask with the opacity. The resulting value should lie in
       range 0 .. 256 */
    if ( msk )
      msk = ((( msk + 1 ) * aOpacity ) >> 8 );

    /* Combine the resulting color with the destination pixel by alpha blending
       or overwrite the pixel in the destination */
    if ( aBlend ) BLEND_RGB565_OPACITY( *dstPtr, src, msk + 1 )
    else          COPY_RGB565_OPACITY ( *dstPtr, src, msk )
...
/* Fill a pixel row with the content of the source surface applying a tint by
   ...
   in the destination memory or alpha-blended with the existing pixel in the
   destination memory. */
void EwFilterRGB565TintByOpacity( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  /* RGB565 has no opacity information to mask. This fiter can be ignored. */
  EW_UNUSED_ARG( aDst          );
  EW_UNUSED_ARG( aSrc          );
  EW_UNUSED_ARG( aMask         );
  EW_UNUSED_ARG( aWidth        );
...
/* Fill a pixel row with the content of the source surface applying a tint by
   ...
   in the destination memory or alpha-blended with the existing pixel in the
   destination memory. */
void EwFilterRGB565TintByLuminance( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1  = 0x0FF00000;
  ...
  while ( aWidth-- > 0 )
  {
    ...
    /* Modulate the color value by the luminance of the source pixel and mix
       the resulting color with the original color from the source */
    TINT_RGB565_LUMINANCE( src, clr, src, aIntensity );
...
/* Fill a pixel row with the content of the source surface applying a mask and
   ...
   written in the destination memory or alpha-blended with the existing pixel in
   the destination memory. */
void EwFilterRGB565MaskTintByOpacity( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  /* RGB565 has no opacity information to mask. This fiter can be ignored. */
...
/* Fill a pixel row with the content of the source surface applying a mask and
   ...
   written in the destination memory or alpha-blended with the existing pixel in
   the destination memory. */
void EwFilterRGB565MaskTintByLuminance( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface modulating the pixel
   by an additional opacity value. The pixel will be written in the destination
   memory or alpha-blended with the existing pixel in the destination memory. */
void EwScreenFilterNative( XSurfaceMemory* aDst, XSurfaceMemory* aSrc, 
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1F = 0x0000001F;
  ...
  /* Convert the values from 8 to 6 bit */
  aOpacity >>= 2;
  ...
  while ( aWidth-- > 0 )
  {
    ...
    /* Combine the resulting color with the destination pixel by alpha blending
       or overwrite the pixel in the destination */
    if ( aBlend ) SCREEN_BLEND_NATIVE_OPACITY( *dstPtr, src, aOpacity )
    else          SCREEN_COPY_NATIVE_OPACITY ( *dstPtr, src, aOpacity )
...
/* Fill a pixel row with the content of the source surface applying a mask
   ...
   destination memory or alpha-blended with the existing pixel in the destination
   memory. */
void EwScreenFilterNativeMask( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1F = 0x0000001F;
  ...
  while ( aWidth-- > 0 )
  {
    ...
    /* Modulate the mask with the opacity. The resulting value should lie in
       range 0 .. 64 */
    if ( msk )
      msk = ((( msk + 1 ) * aOpacity ) >> 10 ) + 1;

    /* Combine the resulting color with the destination pixel by alpha blending
       or overwrite the pixel in the destination */
    if ( aBlend ) SCREEN_BLEND_NATIVE_OPACITY( *dstPtr, src, msk )
    else          SCREEN_COPY_NATIVE_OPACITY ( *dstPtr, src, msk )
...
/* Fill a pixel row with the content of the source surface applying a tint by
   ...
   in the destination memory or alpha-blended with the existing pixel in the
   destination memory. */
void EwScreenFilterNativeTintByOpacity( XSurfaceMemory* aDst, 
  XSurfaceMemory* aSrc, XSurfaceMemory* aMask, int aWidth, int aMaskInverted,
  XGradient* aColor, int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1  = 0x0FF00000;
  unsigned int    mask2  = 0x00FF00FF;
  unsigned int    mask1F = 0x0000001F;
  ...
  /* Stretch the values from range 0..255 to 0..256 (or 0..63 to 0..64) to
     allow shift operations instead of divisions */
  aIntensity++;
  aOpacity++;
...
/* Fill a pixel row with the content of the source surface applying a tint by
   ...
   in the destination memory or alpha-blended with the existing pixel in the
   destination memory. */
void EwScreenFilterNativeTintByLuminance( XSurfaceMemory* aDst, 
  XSurfaceMemory* aSrc, XSurfaceMemory* aMask, int aWidth, int aMaskInverted,
  XGradient* aColor, int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a mask and
   ...
   written in the destination memory or alpha-blended with the existing pixel in
   the destination memory. */
void EwScreenFilterNativeMaskTintByOpacity( XSurfaceMemory* aDst, 
  XSurfaceMemory* aSrc, XSurfaceMemory* aMask, int aWidth, int aMaskInverted,
  XGradient* aColor, int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a mask and
   ...
   written in the destination memory or alpha-blended with the existing pixel in
   the destination memory. */
void EwScreenFilterNativeMaskTintByLuminance( XSurfaceMemory* aDst,
  XSurfaceMemory* aSrc, XSurfaceMemory* aMask, int aWidth, int aMaskInverted,
  XGradient* aColor, int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a mask
   ...
   destination memory or alpha-blended with the existing pixel in the destination
   memory. */
void EwScreenFilterIndex8Mask( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1F = 0x0000001F;
...
/* Fill a pixel row with the content of the source surface applying a tint by
   ...
   in the destination memory or alpha-blended with the existing pixel in the
   destination memory. */
void EwScreenFilterIndex8TintByOpacity( XSurfaceMemory* aDst, 
  XSurfaceMemory* aSrc, XSurfaceMemory* aMask, int aWidth, int aMaskInverted,
  XGradient* aColor, int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a tint by
   ...
   in the destination memory or alpha-blended with the existing pixel in the
   destination memory. */
void EwScreenFilterIndex8TintByLuminance( XSurfaceMemory* aDst, 
  XSurfaceMemory* aSrc, XSurfaceMemory* aMask, int aWidth, int aMaskInverted,
  XGradient* aColor, int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a mask and
   ...
   written in the destination memory or alpha-blended with the existing pixel in
   the destination memory. */
void EwScreenFilterIndex8MaskTintByOpacity( XSurfaceMemory* aDst, 
  XSurfaceMemory* aSrc, XSurfaceMemory* aMask, int aWidth, int aMaskInverted,
  XGradient* aColor, int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a mask and
   ...
   written in the destination memory or alpha-blended with the existing pixel in
   the destination memory. */
void EwScreenFilterIndex8MaskTintByLuminance( XSurfaceMemory* aDst, 
  XSurfaceMemory* aSrc, XSurfaceMemory* aMask, int aWidth, int aMaskInverted, 
  XGradient* aColor, int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a mask
   ...
   destination memory or alpha-blended with the existing pixel in the destination
   memory. */
void EwScreenFilterAlpha8Mask( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  /* ALPHA8 has no color information to mask. This fiter can be ignored. */
...
/* Fill a pixel row with the content of the source surface applying a tint by
   ...
   in the destination memory or alpha-blended with the existing pixel in the
   destination memory. */
void EwScreenFilterAlpha8TintByOpacity( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a tint by
   ...
   in the destination memory or alpha-blended with the existing pixel in the
   destination memory. */
void EwScreenFilterAlpha8TintByLuminance( XSurfaceMemory* aDst, 
  XSurfaceMemory* aSrc, XSurfaceMemory* aMask, int aWidth, int aMaskInverted,
  XGradient* aColor, int aIntensity, int aOpacity, int aBlend )
{
  /* ALPHA8 has luminance == 1. This filter can be ignored. */
...
/* Fill a pixel row with the content of the source surface applying a mask and
   ...
   written in the destination memory or alpha-blended with the existing pixel in
   the destination memory. */
void EwScreenFilterAlpha8MaskTintByOpacity( XSurfaceMemory* aDst, 
  XSurfaceMemory* aSrc, XSurfaceMemory* aMask, int aWidth, int aMaskInverted, 
  XGradient* aColor, int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a mask and
   ...
   written in the destination memory or alpha-blended with the existing pixel in
   the destination memory. */
void EwScreenFilterAlpha8MaskTintByLuminance( XSurfaceMemory* aDst, 
  XSurfaceMemory* aSrc, XSurfaceMemory* aMask, int aWidth, int aMaskInverted, 
  XGradient* aColor, int aIntensity, int aOpacity, int aBlend )
{
  /* ALPHA8 has luminance == 1. This filter can be ignored. */
...
/* Fill a pixel row with the content of the source surface applying a mask
   ...
   destination memory or alpha-blended with the existing pixel in the destination
   memory. */
void EwScreenFilterRGB565Mask( XSurfaceMemory* aDst, XSurfaceMemory* aSrc,
  XSurfaceMemory* aMask, int aWidth, int aMaskInverted, XGradient* aColor,
  int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask2L = 0x0000F81F;
  ...
  while ( aWidth-- > 0 )
  {
    ...
    /* Combine the resulting color with the destination pixel by alpha blending
       or overwrite the pixel in the destination */
    if ( aBlend ) SCREEN_BLEND_RGB565_OPACITY( *dstPtr, src, msk )
    else          SCREEN_COPY_RGB565_OPACITY ( *dstPtr, src, msk )
...
/* Fill a pixel row with the content of the source surface applying a tint by
   ...
   in the destination memory or alpha-blended with the existing pixel in the
   destination memory. */
void EwScreenFilterRGB565TintByOpacity( XSurfaceMemory* aDst, 
  XSurfaceMemory* aSrc, XSurfaceMemory* aMask, int aWidth, int aMaskInverted,
  XGradient* aColor, int aIntensity, int aOpacity, int aBlend )
{
  /* RGB565 has no opacity information to mask. This fiter can be ignored. */
...
/* Fill a pixel row with the content of the source surface applying a tint by
   ...
   in the destination memory or alpha-blended with the existing pixel in the
   destination memory. */
void EwScreenFilterRGB565TintByLuminance( XSurfaceMemory* aDst, 
  XSurfaceMemory* aSrc, XSurfaceMemory* aMask, int aWidth, int aMaskInverted,
  XGradient* aColor, int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1  = 0x0FF00000;
...
/* Fill a pixel row with the content of the source surface applying a mask and
   ...
   written in the destination memory or alpha-blended with the existing pixel in
   the destination memory. */
void EwScreenFilterRGB565MaskTintByOpacity( XSurfaceMemory* aDst, 
  XSurfaceMemory* aSrc, XSurfaceMemory* aMask, int aWidth, int aMaskInverted, 
  XGradient* aColor, int aIntensity, int aOpacity, int aBlend )
{
  /* RGB565 has no opacity information to mask. This fiter can be ignored. */
...
/* Fill a pixel row with the content of the source surface applying a mask and
   ...
   written in the destination memory or alpha-blended with the existing pixel in
   the destination memory. */
void EwScreenFilterRGB565MaskTintByLuminance( XSurfaceMemory* aDst, 
  XSurfaceMemory* aSrc, XSurfaceMemory* aMask, int aWidth, int aMaskInverted, 
  XGradient* aColor, int aIntensity, int aOpacity, int aBlend )
{
  unsigned int    mask1  = 0x0FF00000;
...
/* Fill a pixel column with the content of the source surface applying a blur
   filter with radius specified in aInfo data structure and weighting all pixel
   values by equal factor. This corresponds to the box-blur. The new pixel will
   be written in the destination memory or alpha-blended with the existing pixel
   in the destination memory. The resulting color values can additionally be
   modulated by the opacity value. */
void EwFilterNativeBlur( XSurfaceMemory* aDst, XSurfaceMemory* aSrc, 
  int aCount, int aLimit1, int aLimit2, int aClamp1, int aClamp2, int aRadius,
  int aOpacity, int aBlend )
{
  unsigned int  mask2   = 0x00FF00FF;
  unsigned int* dstPtr  = aDst->Pixel1;
  unsigned int* srcPtr  = aSrc->Pixel1;
  unsigned int* srcPtr1 = srcPtr + aLimit1;
  unsigned int* srcPtr2 = srcPtr + aLimit2 - 1;
  unsigned int  c1      = 0;
  unsigned int  c2      = 0;
  unsigned int  c3      = 0;
  unsigned int  c4      = 0;

  /* To avoid costly divisions cache the last calculated radius inverse */
  static int cachedRadius = 0;
  static int cachedInv    = 0;

  /* To avoid costly divisions we multiply with 1 / tap_size where tap_size is
     radius + 1 + radius */
  if ( aRadius != cachedRadius )
  {
    cachedRadius = aRadius;
    cachedInv    = ( 65536 / ( aRadius + 1 + aRadius )) + 1;
  }

  /* Estimate the range of source pixel on the left and on the right of the
     start position to accumulate. This corresponds to the blur radius unless
     we are already at the edges of the source surface (aLimit1 and aLimit2). */
  aLimit1 = MAX( -aRadius,     aLimit1 );
  aLimit2 = MIN(  aRadius + 1, aLimit2 );

  /* Accumulate the values of the pixel lying on the left/right of the first
     pixel */
  for ( ; aLimit1 < aLimit2; aLimit1++ )
  {
    unsigned int c  = *MAP_UINT32( srcPtr + aLimit1 );
    unsigned int t1 = ( c >>  0 ) & 0xFF;
    unsigned int t2 = ( c >>  8 ) & 0xFF;
    unsigned int t3 = ( c >> 16 ) & 0xFF;
    unsigned int t4 = ( c >> 24 );

    /* Ensure, the pixel are alpha pre-multiplied */
    PREMULTIPLY_NATIVE_CHANNELS( t1, t2, t3, t4 );
  ...
  /* Repeat until the entire sequence is processed */
  while ( aCount-- > 0 )
  {
    unsigned int* tmpSrcPtr1 = srcPtr - aRadius;
    unsigned int* tmpSrcPtr2 = srcPtr + aRadius + 1;
    unsigned int  t1         = c1;
    unsigned int  t2         = c2;
    unsigned int  t3         = c3;
    unsigned int  t4         = c4;
    ...
    /* Processing pixel near to the clamp edge? In such case all pixel lying
       beyond the clamp edge have to be excluded as if they were not existing.
       This means, we count only the existing pixel. Estimate the inverse
       value of this pixel number. */
    if (( --aClamp2 < aRadius ) || ( aClamp1 < aRadius ))
    {
      int tmp = MIN( aClamp1, aRadius ) + 1 + MIN( aClamp2, aRadius );
      inv     = tmp? 65536 / tmp + 1 : 0;
      aClamp1++;
    }

    /* All pixel are within the clamping area. Use the pre-calculated inverse
       value valid when all pixel are counted. */
    else
      inv = cachedInv;

    /* Divide the accumulated sum by the number of affected pixel. For the
       purpose of optimization we multiply the values by an inverse of the
       divisor. */
    t1 = ( t1 * inv ) >> 16;
    t2 = ( t2 * inv ) >> 16;
    t3 = ( t3 * inv ) >> 16;
    ...
    /* Compose from the channels a single color value */
    src = t1 | ( t2 << 8 ) | ( t3 << 16 ) | ( t4 << 24 );

    /* Combine the resulting color with the destination pixel by alpha blending
       or overwrite the pixel in the destination */
    if ( aBlend ) BLEND_PREMULT_NATIVE_OPACITY( *dstPtr, src, aOpacity )
    else          COPY_PREMULT_NATIVE_OPACITY ( *dstPtr, src, aOpacity )

    /* The source pixel at the left end of the accumulated area leaves the area
       now - subtract the pixel from the sum */
    if (( tmpSrcPtr1 >= srcPtr1 ) && ( tmpSrcPtr1 <= srcPtr2 ))
    {
      unsigned int c  = *MAP_UINT32( tmpSrcPtr1 );
      unsigned int t1 = ( c >>  0 ) & 0xFF;
      unsigned int t2 = ( c >>  8 ) & 0xFF;
      unsigned int t3 = ( c >> 16 ) & 0xFF;
      unsigned int t4 = ( c >> 24 );
    ...
    /* New source pixel at the right end of the accumulated area enters the
       area now - add the pixel to the sum */
    if (( tmpSrcPtr2 >= srcPtr1 ) && ( tmpSrcPtr2 <= srcPtr2 ))
    {
      unsigned int c  = *MAP_UINT32( tmpSrcPtr2 );
      unsigned int t1 = ( c >>  0 ) & 0xFF;
      unsigned int t2 = ( c >>  8 ) & 0xFF;
      unsigned int t3 = ( c >> 16 ) & 0xFF;
      unsigned int t4 = ( c >> 24 );
...
/* Fill a pixel column with the content of the source surface applying a blur
   filter with radius specified in aInfo data structure and weighting all pixel
   values by equal factor. This corresponds to the box-blur. The new pixel will
   be written in the destination memory. */
void EwFilterIndex8Blur( XSurfaceMemory* aDst, XSurfaceMemory* aSrc, 
  int aCount, int aLimit1, int aLimit2, int aClamp1, int aClamp2, int aRadius,
  int aOpacity, int aBlend )
{
  #ifndef EW_PREMULTIPLY_COLOR_CHANNELS
  ...
  unsigned int*  dstPtr  = aDst->Pixel1;
  unsigned char* srcPtr  = aSrc->Pixel1;
  unsigned char* srcPtr1 = srcPtr + aLimit1;
  unsigned char* srcPtr2 = srcPtr + aLimit2 - 1;
  unsigned int*  clut    = aSrc->Clut;
  unsigned int   c1      = 0;
  unsigned int   c2      = 0;
  unsigned int   c3      = 0;
  ...
  for ( ; aLimit1 < aLimit2; aLimit1++ )
  {
    unsigned int c  = *MAP_UINT32( clut + *MAP_UINT8( srcPtr + aLimit1 ));
    unsigned int t1 = ( c >>  0 ) & 0xFF;
    unsigned int t2 = ( c >>  8 ) & 0xFF;
    unsigned int t3 = ( c >> 16 ) & 0xFF;
    unsigned int t4 = ( c >> 24 );
  ...
  while ( aCount-- > 0 )
  {
    unsigned char* tmpSrcPtr1 = srcPtr - aRadius;
    unsigned char* tmpSrcPtr2 = srcPtr + aRadius + 1;
    unsigned int   t1         = c1;
    unsigned int   t2         = c2;
    unsigned int   t3         = c3;
    unsigned int   t4         = c4;
    ...
    /* Overwrite the pixel in the destination */
    COPY_PREMULT_NATIVE( *dstPtr, src );

    /* The source pixel at the left end of the accumulated area leaves the area
       now - subtract the pixel from the sum */
    if (( tmpSrcPtr1 >= srcPtr1 ) && ( tmpSrcPtr1 <= srcPtr2 ))
    {
      unsigned int c  = *MAP_UINT32( clut + *MAP_UINT8( tmpSrcPtr1 ));
      unsigned int t1 = ( c >>  0 ) & 0xFF;
      unsigned int t2 = ( c >>  8 ) & 0xFF;
      unsigned int t3 = ( c >> 16 ) & 0xFF;
      unsigned int t4 = ( c >> 24 );
    ...
    if (( tmpSrcPtr2 >= srcPtr1 ) && ( tmpSrcPtr2 <= srcPtr2 ))
    {
      unsigned int c  = *MAP_UINT32( clut + *MAP_UINT8( tmpSrcPtr2 ));
      unsigned int t1 = ( c >>  0 ) & 0xFF;
      unsigned int t2 = ( c >>  8 ) & 0xFF;
      unsigned int t3 = ( c >> 16 ) & 0xFF;
      unsigned int t4 = ( c >> 24 );
...
/* Fill a pixel column with the content of the source surface applying a blur
   ...
   values by equal factor. This corresponds to the box-blur. The new pixel will
   be written in the destination memory. */
void EwAlpha8FilterAlpha8Blur( XSurfaceMemory* aDst, XSurfaceMemory* aSrc, 
  int aCount, int aLimit1, int aLimit2, int aClamp1, int aClamp2, int aRadius,
  int aOpacity, int aBlend )
{
  unsigned char* dstPtr  = aDst->Pixel1;
  unsigned char* srcPtr  = aSrc->Pixel1;
  unsigned char* srcPtr1 = srcPtr + aLimit1;
  unsigned char* srcPtr2 = srcPtr + aLimit2 - 1;
  unsigned int   c1      = 0;
  ...
  while ( aCount-- > 0 )
  {
    ...
    /* Processing pixel near to the clamp edge? In such case all pixel lying
       beyond the clamp edge have to be excluded as if they were not existing.
       This means, we count only the existing pixel. */
    if (( --aClamp2 < aRadius ) || ( aClamp1 < aRadius ))
    {
      int tmp = MIN( aClamp1, aRadius ) + 1 + MIN( aClamp2, aRadius );
      if ( tmp )
    ...
    /* All pixel are within the clamping area. Use the pre-calculated inverse
      ...
      src = ( src * cachedInv ) >> 16;

    /* Store the blurred ALPHA8 value */
    *dstPtr = (unsigned char)src;

    /* The source pixel at the left end of the accumulated area leaves the area
       now - subtract the pixel from the sum */
    if (( tmpSrcPtr1 >= srcPtr1 ) && ( tmpSrcPtr1 <= srcPtr2 ))
      c1 -= *MAP_UINT8( tmpSrcPtr1 );

    /* New source pixel at the right end of the accumulated area enters the
       area now - add the pixel to the sum */
    if (( tmpSrcPtr2 >= srcPtr1 ) && ( tmpSrcPtr2 <= srcPtr2 ))
      c1 += *MAP_UINT8( tmpSrcPtr2 );
...
/* Fill a pixel column with the content of the source surface applying a blur
   ...
   values by equal factor. This corresponds to the box-blur. The new pixel will
   be written in the destination memory. */
void EwFilterRGB565Blur( XSurfaceMemory* aDst, XSurfaceMemory* aSrc, 
  int aCount, int aLimit1, int aLimit2, int aClamp1, int aClamp2, int aRadius,
  int aOpacity, int aBlend )
{
  #ifndef EW_PREMULTIPLY_COLOR_CHANNELS
  ...
  unsigned int*   dstPtr  = aDst->Pixel1;
  unsigned short* srcPtr  = aSrc->Pixel1;
  unsigned short* srcPtr1 = srcPtr + aLimit1;
  unsigned short* srcPtr2 = srcPtr + aLimit2 - 1;
  unsigned int    c1      = 0;
  unsigned int    c2      = 0;
  unsigned int    c3      = 0;
  unsigned int    c4      = 0;
  ...
  for ( ; aLimit1 < aLimit2; aLimit1++ )
  {
    unsigned int c  = *MAP_UINT16( srcPtr + aLimit1 );
    c1 += ( c >>  0 ) & 0x1F; c2 += ( c >>  5 ) & 0x3F;
    c3 += ( c >> 11 ) & 0x1F; c4 += 255;
  }
  ...
  while ( aCount-- > 0 )
  {
    unsigned short* tmpSrcPtr1 = srcPtr - aRadius;
    unsigned short* tmpSrcPtr2 = srcPtr + aRadius + 1;
    unsigned int    t1         = c1;
    unsigned int    t2         = c2;
    unsigned int    t3         = c3;
    unsigned int    t4         = c4;
    ...
    /* Convert RGB565 to NATIVE. Alpha channel is already in NATIVE format */
    t1 = ( t1 << 3 ) | ( t1 >> 2 );
    t2 = ( t2 << 2 ) | ( t2 >> 4 );
    t3 = ( t3 << 3 ) | ( t3 >> 2 );

    /* Compose from the channels a single NATIVE color value */
    #if EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_RED == 0
      src = ( t1 << EW_COLOR_CHANNEL_BIT_OFFSET_RED   ) | 
            ( t2 << EW_COLOR_CHANNEL_BIT_OFFSET_GREEN ) |
            ( t3 << EW_COLOR_CHANNEL_BIT_OFFSET_BLUE  ) |
            ( t4 << EW_COLOR_CHANNEL_BIT_OFFSET_ALPHA );
    #else
      src = ( t3 << EW_COLOR_CHANNEL_BIT_OFFSET_RED   ) | 
            ( t2 << EW_COLOR_CHANNEL_BIT_OFFSET_GREEN ) |
            ( t1 << EW_COLOR_CHANNEL_BIT_OFFSET_BLUE  ) |
            ( t4 << EW_COLOR_CHANNEL_BIT_OFFSET_ALPHA );
    #endif
    ...
    if (( tmpSrcPtr1 >= srcPtr1 ) && ( tmpSrcPtr1 <= srcPtr2 ))
    {
      unsigned int c = *MAP_UINT16( tmpSrcPtr1 );
      c1 -= ( c >>  0 ) & 0x1F; c2 -= ( c >>  5 ) & 0x3F;
      c3 -= ( c >> 11 ) & 0x1F; c4 -= 255;
    }
    ...
    if (( tmpSrcPtr2 >= srcPtr1 ) && ( tmpSrcPtr2 <= srcPtr2 ))
    {
      unsigned int c = *MAP_UINT16( tmpSrcPtr2 );
      c1 += ( c >>  0 ) & 0x1F; c2 += ( c >>  5 ) & 0x3F;
      c3 += ( c >> 11 ) & 0x1F; c4 += 255;
    }
...
/* Fill a pixel column with the content of the source surface applying a blur
   ...
   in the destination memory. The resulting color values can additionally be
   modulated by the opacity value. */
void EwScreenFilterNativeBlur( XSurfaceMemory* aDst, XSurfaceMemory* aSrc, 
  int aCount, int aLimit1, int aLimit2, int aClamp1, int aClamp2, int aRadius,
  int aOpacity, int aBlend )
{
  unsigned int    mask1F  = 0x0000001F;
  ...
  while ( aCount-- > 0 )
  {
    ...
    /* Combine the resulting color with the destination pixel by alpha blending
       or overwrite the pixel in the destination */
    if ( aBlend ) SCREEN_BLEND_PREMULT_NATIVE_OPACITY( *dstPtr, src, aOpacity )
    else          SCREEN_COPY_PREMULT_NATIVE_OPACITY ( *dstPtr, src, aOpacity )
...
/* The following function defines the low-level, pixel-wise function to fill
   ...
   destination. */
void EwFillLinearGradientRow( XSurfaceMemory* aDst, XSurfaceMemory* aMask,
  int aWidth, int aDeltaX, int aDeltaY, int aUnitX, int aUnitY, int aNoOfStops,
  int* aOffsets, unsigned int* aColors )
{
  unsigned int   mask2  = 0x00FF00FF;
  unsigned int*  dstPtr = aDst->Pixel1;
  unsigned char* mskPtr = aMask->Pixel1;
  ...
  /* Process all pixel */
  for ( ; aWidth > 0; aWidth--, aDeltaX++, mskPtr++, dstPtr++ )
  {
    unsigned int opacity = *mskPtr + 1;
    unsigned int clr;
    unsigned int dst;
    int          ofs;

    /* Calculate the position at the gradient line relative to gradient's start
       position. */
    ofs = (( aDeltaX * aUnitX ) + ( aDeltaY * aUnitY ) + ( 1 << 8 )) >> 16;

    /* The position lies before the first color stop */
    if ( ofs < aOffsets[0])
      clr = aColors[0];

    /* The position lies after the last color stop */
    else if ( ofs >= aOffsets[ aNoOfStops - 1 ] )
      clr = aColors[ aNoOfStops - 1 ];

    /* The position lies within the gradient. Find the corresponding section */
    else
    {
      unsigned int f;
      int          i;

      /* Search for the pair of two color stops enclosing the offset */
      for ( i = 1; i < aNoOfStops; i++ )
        if ( ofs < aOffsets[i])
          break;

      /* Calculate a factor in range 0 .. 256 describing the relative position
         between the color stops */
      f  = ((( ofs - aOffsets[i-1] ) * 256 ) / ( aOffsets[i] - aOffsets[i-1] ));

      /* Interpolate between the color values */
      MIX_PACKED_COLORS( clr, aColors[i-1], aColors[i], f );
    }

    /* Get the native representation of the interpolated, packed 32- bit color
       value and store the results in destination */
    PACKED_COLOR_2_NATIVE( dst, clr );
    COPY_NATIVE_OPACITY( *dstPtr, dst, opacity );
  }
}
...
void EwFillLinearGradientRowBlend( XSurfaceMemory* aDst, XSurfaceMemory* aMask,
  int aWidth, int aDeltaX, int aDeltaY, int aUnitX, int aUnitY, int aNoOfStops,
  int* aOffsets, unsigned int* aColors )
{
  unsigned int   mask2  = 0x00FF00FF;
  ...
  for ( ; aWidth > 0; aWidth--, aDeltaX++, mskPtr++, dstPtr++ )
  {
    ...
    /* Get the native representation of the interpolated, packed 32- bit color
       value and blend it with the pixel existing in the destination */
    PACKED_COLOR_2_NATIVE( dst, clr );
    BLEND_NATIVE_OPACITY( *dstPtr, dst, opacity );
  }
}
...
/* The following function defines the low-level, pixel-wise function to fill
   ...
   ALPHA8 pixel row. The new pixel overwrites the existing pixel within the
   destination. */
void EwFillRadialGradientRow( XSurfaceMemory* aDst, XSurfaceMemory* aMask,
  int aWidth, int aDeltaX, int aDeltaY, int* aMatrix, int aNoOfStops,
  int* aOffsets, unsigned int* aColors )
{
  unsigned int   mask2  = 0x00FF00FF;
  unsigned int*  dstPtr = aDst->Pixel1;
  ...
  for ( ; aWidth > 0; aWidth--, aDeltaX++, mskPtr++, dstPtr++ )
  {
    ...
    /* Apply scaling/rotation to the coordinates? */
    if ( aMatrix )
    {
      dx  = (( aDeltaX * aMatrix[0]) + ( aDeltaY * aMatrix[1])) >> 12;
      dy  = (( aDeltaX * aMatrix[2]) + ( aDeltaY * aMatrix[3])) >> 12;
      ofs = (int)( EwMathSqrt((float)(( dx * dx ) + ( dy * dy ))) * 16.0f );
    }

    /* Calculate the position at the gradient radius relative to circle's center
       position. */
    else
      ofs = (int)( EwMathSqrt((float)(( dx * dx ) + ( dy * dy ))) * 256.0f );
    ...
    /* The position lies after the last color stop */
    else if ( ofs >= ( aOffsets[ aNoOfStops - 1 ] << 8 ))
      clr = aColors[ aNoOfStops - 1 ];
    ...
    else
    {
      ...
      /* Calculate a factor in range 0 .. 256 describing the relative position
         between the color stops */
      f  = ((( ofs - ( aOffsets[i-1] << 8 ))) / ( aOffsets[i] - aOffsets[i-1] ));
...
/* The following function defines the low-level, pixel-wise function to fill
   ...
   ALPHA8 pixel row. The new pixel will be alpha blended with the existing
   pixel of the destination. */
void EwFillRadialGradientRowBlend( XSurfaceMemory* aDst, XSurfaceMemory* aMask,
  int aWidth, int aDeltaX, int aDeltaY, int* aMatrix, int aNoOfStops,
  int* aOffsets, unsigned int* aColors )
{
  unsigned int   mask2  = 0x00FF00FF;
...
/* The following function defines the low-level, pixel-wise function to fill
   ...
   ALPHA8 pixel row. The new pixel overwrites the existing pixel within the
   destination. */
void EwScreenFillLinearGradientRow( XSurfaceMemory* aDst, XSurfaceMemory* aMask,
  int aWidth, int aDeltaX, int aDeltaY, int aUnitX, int aUnitY, int aNoOfStops,
  int* aOffsets, unsigned int* aColors )
{
  unsigned int    mask2L = 0x0000F81F;
  ...
  for ( ; aWidth > 0; aWidth--, aDeltaX++, mskPtr++, dstPtr++ )
  {
    unsigned int opacity = ( *mskPtr >> 2 ) + 1;
    unsigned int clr;
    unsigned int dst;
    int          ofs;
    ...
    /* Get the native representation of the interpolated, packed 32- bit color
       value and store the results in destination */
    PACKED_COLOR_2_NATIVE( dst, clr );
    SCREEN_COPY_NATIVE_OPACITY( *dstPtr, dst, opacity );
  }
}
...
/* The following function defines the low-level, pixel-wise function to fill
   ...
   ALPHA8 pixel row. The new pixel will be alpha blended with the existing
   pixel of the destination. */
void EwScreenFillLinearGradientRowBlend( XSurfaceMemory* aDst, XSurfaceMemory* aMask,
  int aWidth, int aDeltaX, int aDeltaY, int aUnitX, int aUnitY, int aNoOfStops,
  int* aOffsets, unsigned int* aColors )
{
  unsigned int    mask1F = 0x0000001F;
  ...
  for ( ; aWidth > 0; aWidth--, aDeltaX++, mskPtr++, dstPtr++ )
  {
    ...
    /* Get the native representation of the interpolated, packed 32- bit color
       value and blend it with the pixel existing in the destination */
    PACKED_COLOR_2_NATIVE( dst, clr );
    SCREEN_BLEND_NATIVE_OPACITY( *dstPtr, dst, opacity );
  }
}
...
/* The following function defines the low-level, pixel-wise function to fill
   ...
   ALPHA8 pixel row. The new pixel overwrites the existing pixel within the
   destination. */
void EwScreenFillRadialGradientRow( XSurfaceMemory* aDst, XSurfaceMemory* aMask,
  int aWidth, int aDeltaX, int aDeltaY, int* aMatrix, int aNoOfStops,
  int* aOffsets, unsigned int* aColors )
{
  unsigned int    mask2L = 0x0000F81F;
...
/* The following function defines the low-level, pixel-wise function to fill
   ...
   ALPHA8 pixel row. The new pixel will be alpha blended with the existing
   pixel of the destination. */
void EwScreenFillRadialGradientRowBlend( XSurfaceMemory* aDst, XSurfaceMemory* aMask,
  int aWidth, int aDeltaX, int aDeltaY, int* aMatrix, int aNoOfStops,
  int* aOffsets, unsigned int* aColors )
{
  unsigned int    mask1F = 0x0000001F;
...
/* Following internal function obtains color format specific information and
   stores it in aInfo data structure. */
void EwGetColorFormatSpecificInfo( XGraphicsEngineInfo* aInfo )
{
  aInfo->ColorChannelBitOffsetRed   = EW_COLOR_CHANNEL_BIT_OFFSET_RED;
  aInfo->ColorChannelBitOffsetGreen = EW_COLOR_CHANNEL_BIT_OFFSET_GREEN;
  aInfo->ColorChannelBitOffsetBlue  = EW_COLOR_CHANNEL_BIT_OFFSET_BLUE;
  aInfo->ColorChannelBitOffsetAlpha = EW_COLOR_CHANNEL_BIT_OFFSET_ALPHA;

  #ifdef EW_PREMULTIPLY_COLOR_CHANNELS
    aInfo->PremultiplyColorChannels   = 1;
  #endif

  aInfo->ScreenColorChannelBitOffsetRed   = EW_SCREEN_COLOR_CHANNEL_BIT_OFFSET_RED;
  aInfo->ScreenColorChannelBitOffsetGreen = EW_SCREEN_COLOR_CHANNEL_BIT_OFFSET_GREEN;
  aInfo->ScreenColorChannelBitOffsetBlue  = EW_SCREEN_COLOR_CHANNEL_BIT_OFFSET_BLUE;
  aInfo->RGB565ColorChannelBitOffsetRed   = EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_RED;
  aInfo->RGB565ColorChannelBitOffsetGreen = EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_GREEN;
  aInfo->RGB565ColorChannelBitOffsetBlue  = EW_RGB565_COLOR_CHANNEL_BIT_OFFSET_BLUE;
}
