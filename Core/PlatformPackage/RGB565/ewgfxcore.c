...
int       EwMaxNoOfGlyphs         = EW_MAX_NO_OF_GLYPHS;
...
/* The following helper function has the job to release the glyph surface
   area occupied by the glyph. */
static void UnmapGlyph( XGlyph* aGlyph );
...
static int FlushAndClean( int aSurfaces, int aGlyphs, int aMappedGlyphs );
...
/* The following heper function verifies whether the memory size of all surfaces
   ...
   aForce == 1 all not-cachable and actually not used surfaces are freed. If
   aForce == 2 all unused surfaces are freed regardless of whether these are
   cachable or not. The function returns != 0 if it could at least free one
   surface. */
...
EW_PERF_COUNTER( EwGfxCreateSurfaceWrapped, Platform_Integration_API )
...
int EwInitGfxCore( void )
{
  ...
  EwAddPerfCounter( EwGfxCreateSurfaceWrapped );
  ...
  /* The granularity determines the size of a single glyph surface cell
     and the resulting number of rows and columns within the cache. Calculate
     the number of rows and columns so that the entire surface cache area
     does not exceed the originally configured value = EwMaxGlyphSurfaceWidth
     * EwMaxGlyphSurfaceHeight. This mreans: the dimension of the resulting
     glyph cache surface may differ from the pre-configured value. */
  GlyphCacheCellSize = ( EwMaxGlyphSurfaceHeight + NoOfGlyphCacheRows - 1 ) /
                         NoOfGlyphCacheRows;
  NoOfGlyphCacheCols = ((( EwMaxGlyphSurfaceWidth * EwMaxGlyphSurfaceHeight ) /
                         ( NoOfGlyphCacheRows * GlyphCacheCellSize )) +
                           GlyphCacheCellSize - 1 ) / GlyphCacheCellSize;
  ...
  /* The maximum number of glyphs has not been specified. Estimate the value
     automatically based on the size of the glyph surface cache. For this 
     purpose we assume an average glyph has the size of 12 x 12 pixel. An
     additional reserve of 8 glyphs is taken account. */
  if ( !EwMaxNoOfGlyphs )
    EwMaxNoOfGlyphs = (( EwMaxGlyphSurfaceHeight * EwMaxGlyphSurfaceWidth ) /
                       ( 12 * 12 )) + 8;

  /* Estimate a number of lists to sort the cached glyphs ... It depends on
     the number how many glyphs can be retained in the cache. The more glyphs,
     more lists are needed to avoid long searching operations. The number of
     the list is a hash extracted from the glyph-id by using a mask */
  if      ( EwMaxNoOfGlyphs >  4096 ) NoOfGlyphCacheLists = 256;
  else if ( EwMaxNoOfGlyphs >= 2048 ) NoOfGlyphCacheLists = 128;
  else if ( EwMaxNoOfGlyphs >= 1024 ) NoOfGlyphCacheLists = 64;
  else if ( EwMaxNoOfGlyphs >=  512 ) NoOfGlyphCacheLists = 32;
  else if ( EwMaxNoOfGlyphs >=  256 ) NoOfGlyphCacheLists = 16;
  else if ( EwMaxNoOfGlyphs >=  128 ) NoOfGlyphCacheLists = 8;
  else if ( EwMaxNoOfGlyphs >=   64 ) NoOfGlyphCacheLists = 4;
  else                                NoOfGlyphCacheLists = 2;

  /* Set of up to 256 lists to store glyphs sorted by their glyph-id */
  ...
  void* aTag1, void* aTag2 )
{
  ...
  while ( !surface && ( FlushAndClean( 1, 1, 0 ) || EwImmediateReclaimMemory( 37 )));
  ...
  while ( !handle && FlushAndClean( 1, 0, 0 ));
...
XSurface* EwCreateConstSurface( int aFormat, int aWidth, int aHeight,
  void* aOwner, void* aTag1, void* aTag2, 
  XSurfaceMemory* aMemory )
{
  ...
  do
    surface = EwAlloc( sizeof( XSurface ));
  while ( !surface && ( FlushAndClean( 1, 1, 0 ) || EwImmediateReclaimMemory( 38 )));
...
*
*******************************************************************************/
XSurface* EwCreateSurfaceWrapped( int aFormat, int aWidth, int aHeight,
  void* aOwner, void* aTag1, void* aTag2, void* aHandle,
  XDeleteSurfaceProc aDeleteProc, void* aDeleteArg )
{
  XSurface*     surface = 0;
  void*         handle  = 0;

  EW_UNUSED_ARG ( aHandle );
  EW_UNUSED_ARG ( aDeleteProc );
  EW_UNUSED_ARG ( aDeleteArg );
  ...
  do
    surface = EwAlloc( sizeof( XSurface ));
  while ( !surface && ( FlushAndClean( 1, 1, 0 ) || EwImmediateReclaimMemory( 36 )));
  ...
  do
  {
    EwStartPerfCounter( EwGfxCreateSurfaceWrapped );
    {
      if ( aFormat == EW_PIXEL_FORMAT_NATIVE )
        #ifdef EwGfxCreateNativeSurfaceWrapped
          handle = EwGfxCreateNativeSurfaceWrapped( aFormat, aWidth, aHeight, 
                                                    aHandle, aDeleteProc, 
                                                    aDeleteArg );
        #else
        {
      ...
      if ( aFormat == EW_PIXEL_FORMAT_ALPHA8 )
        #ifdef EwGfxCreateAlpha8SurfaceWrapped
          handle = EwGfxCreateAlpha8SurfaceWrapped( aFormat, aWidth, aHeight, 
                                                    aHandle, aDeleteProc, 
                                                    aDeleteArg );
        #else
        {
      ...
      if ( aFormat == EW_PIXEL_FORMAT_INDEX8 )
        #ifdef EwGfxCreateIndex8SurfaceWrapped
          handle = EwGfxCreateIndex8SurfaceWrapped( aFormat, aWidth, aHeight, 
                                                    aHandle, aDeleteProc, 
                                                    aDeleteArg );
        #else
        {
      ...
      if ( aFormat == EW_PIXEL_FORMAT_RGB565 )
        #ifdef EwGfxCreateRGB565SurfaceWrapped
          handle = EwGfxCreateRGB565SurfaceWrapped( aFormat, aWidth, aHeight, 
                                                    aHandle, aDeleteProc, 
                                                    aDeleteArg );
        #else
        {
    ...
    EwStopPerfCounter( EwGfxCreateSurfaceWrapped );
  }
  while ( !handle && FlushAndClean( 1, 0, 0 ));
  ...
  #ifdef EW_SUPPORT_GFX_TASK_TRACING
    if ( EwPrintGfxTasks )
      EwPrint( "CreateSurfaceWrapped( %s, %d, %d ) --> 0x%p\n", 
               ( aFormat == EW_PIXEL_FORMAT_NATIVE )? "NATIVE" :
               ( aFormat == EW_PIXEL_FORMAT_ALPHA8 )? "ALPHA8" :
               ( aFormat == EW_PIXEL_FORMAT_RGB565 )? "RGB565" : "INDEX8",
               aWidth, aHeight, surface );
...
*
*******************************************************************************/
void EwSetSurfaceCacheTags( XSurface* aSurface, void* aTag1, 
  void* aTag2, XDiscardSurfaceProc aDiscardProc )
{
  aSurface->Tag1        = aTag1;
  aSurface->Tag2        = aTag2;
  aSurface->DiscardProc = (void*)aDiscardProc;

  /* Does it make sense to retain the surface in cache after being freed? */
  #ifndef EW_CACHE_OFFSCREEN_SURFACES  
    aSurface->DoCache = aTag1 || aTag2;
  #endif
}
  ...
  EwStartPerfCounter( EwGfxLockSurface );
  {
      ...
      if ( aSurface->Format == EW_PIXEL_FORMAT_SCREEN )
          ...
          lockHandle = EwGfxLockScreenSurface( handle, aX, aY, aWidth, aHeight, 0,
                         ...
                         0, 0, mem );
        #else
          lockHandle = (void*)(unsigned long)EwGetScreenSurfaceMemory( handle,
                               aX, aY, 0, aMode & EW_LOCK_PIXEL_WRITE, mem );
        #endif
    #endif
    ...
    if ( aSurface->Format == EW_PIXEL_FORMAT_NATIVE )
        ...
        lockHandle = EwGfxLockNativeSurface( handle, aX, aY, aWidth, aHeight, 0,
                       ...
                       0, 0, mem );
      #else
        lockHandle = (void*)(unsigned long)EwGetNativeSurfaceMemory( handle,
                             aX, aY, 0, aMode & EW_LOCK_PIXEL_WRITE, mem );
      #endif
    ...
    if ( aSurface->Format == EW_PIXEL_FORMAT_ALPHA8 )
        ...
        lockHandle = EwGfxLockAlpha8Surface( handle, aX, aY, aWidth, aHeight, 0,
                       ...
                       0, 0, mem );
      #else
        lockHandle = (void*)(unsigned long)EwGetAlpha8SurfaceMemory( handle,
                             aX, aY, 0, aMode & EW_LOCK_PIXEL_WRITE, mem );
      #endif
    ...
    if ( aSurface->Format == EW_PIXEL_FORMAT_INDEX8 )
      ...
      #else
        lockHandle = (void*)(unsigned long)EwGetIndex8SurfaceMemory( handle,
                             aX, aY, aIndex, ( aMode & EW_LOCK_PIXEL_WRITE ) ||
                                             ( aMode & EW_LOCK_CLUT_WRITE  ), mem );
      #endif
    ...
    if ( aSurface->Format == EW_PIXEL_FORMAT_RGB565 )
        ...
        lockHandle = EwGfxLockRGB565Surface( handle, aX, aY, aWidth, aHeight, 0,
                       ...
                       0, 0, mem );
      #else
        lockHandle = (void*)(unsigned long)EwGetRGB565SurfaceMemory( handle,
                             aX, aY, 0, aMode & EW_LOCK_PIXEL_WRITE, mem );
      #endif
  }
...
*
*******************************************************************************/
XGlyph* EwCreateGlyph( void* aFont, unsigned int aGlyphId, 
  int aOriginX, int aOriginY, int aWidth, int aHeight, int aAdvance )
{
  ...
  /* Too many glyphs? Discard the oldest glyphs which are not mapped. */
  while (( EwNoOfGlyphs > EwMaxNoOfGlyphs ) && glyph )
  {
    XGlyph* tmp = glyph;

    glyph = glyph->Newer;

    /* Discard unused glyphs and only */
    if ( !tmp->Used )
      DiscardGlyph( tmp );
  }

  /* Maximum number of glyphs has been reached. */
  if ( EwNoOfGlyphs > EwMaxNoOfGlyphs )
    return 0;

  /* Get memory for the glyph structure. If not possible, try to free the still
     unused resources and repeat */
  do
    glyph = EwAlloc( sizeof( XGlyph ));
  while ( !glyph && ( FlushAndClean( 1, 1, 0 ) || EwImmediateReclaimMemory( 36 )));
  ...
  glyph->Font     = aFont;
  glyph->GlyphId  = aGlyphId;
  glyph->Width    = (short)aWidth;
  glyph->Height   = (short)aHeight;
  glyph->OriginX  = (short)aOriginX;
  glyph->OriginY  = (short)aOriginY;
  glyph->Advance  = (short)aAdvance;

  /* For the moment the glyph is not loaded into the glyph surface. */
  glyph->CacheX   = -1;
  glyph->CacheY   = -1;
  ...
  #ifdef EW_SUPPORT_GFX_TASK_TRACING
    if ( EwPrintGfxTasks )
      EwPrint( "CreateGlyph( 0x%08X, %d, %d, %d, %d, %d ) --> 0x%p\n",
               aGlyphId, aWidth, aHeight, aOriginX, aOriginY, aAdvance, glyph );
  #endif
...
*
*******************************************************************************/
int EwMapGlyph( XGlyph* aGlyph, int* aLoad )
{
  int ok;
  int row = 0;
  int col = 0;
  ...
  /* We assume the glyph is already loaded or it does not require any loading
     operation. */
  *aLoad = 0;

  /* Is the glyph already mapped? */
  if (( aGlyph->CacheX >= 0 ) || ( aGlyph->CacheY >= 0 ))
    return 1;

  /* Does the glyph require space in the surface? */
  if (( aGlyph->Width <= 0 ) || ( aGlyph->Height <= 0 ))
    return 1;
  ...
  while ( !ok && ( FlushAndClean( 0, 0, 1 ) || EwImmediateReclaimMemory( 39 )));
  ...
  /* Reserve the area within the glyph surface cache */
  ReserveGlyphCacheArea( col, row, cols, rows );

  /* The glyph occupies now this memory area withinthe glyph surface */
  aGlyph->CacheX = (short)( col * GlyphCacheCellSize );
  aGlyph->CacheY = (short)( row * GlyphCacheCellSize );

  /* Track the biggest glyph */
  if ( aGlyph->Width  > EwMaxGlyphWidth  ) EwMaxGlyphWidth  = aGlyph->Width;
  if ( aGlyph->Height > EwMaxGlyphHeight ) EwMaxGlyphHeight = aGlyph->Height;

  /* Track the operations */
  #ifdef EW_SUPPORT_GFX_TASK_TRACING
    if ( EwPrintGfxTasks )
      EwPrint( "MapGlyph( 0x%p ) --> ( %d, %d )\n",
               aGlyph, aGlyph->CacheX, aGlyph->CacheY );
  #endif

  /* The glyph has been mapped successfully. Now, the pixel data has to be 
     loaded. */
  *aLoad = 1;
  return 1;
}
...
*   the given font and glyph values. If no glyph could be found, the function
...
*
*******************************************************************************/
XGlyph* EwFindGlyph( void* aFont, unsigned int aGlyphId )
{
  /* Depending on the glyph code - select the appropriate hash list */
  XGlyph* glyph = EwGlyphCacheHeads[ aGlyphId & ( NoOfGlyphCacheLists - 1 )];
...
XIssue* EwInitIssue( XSurface* aSurface )
{
  ...
  while ( !issue && ( FlushAndClean( 1, 1, 0 ) || EwImmediateReclaimMemory( 40 )));
...
*
*******************************************************************************/
int EwCleanSurfaceCache( int aFullClean )
{
  return CleanSurfaceCache( aFullClean? 2 : 1 );
}
...
static void DiscardSurface( XSurface* aSurface )
{
  ...
  /* Run cleanup code, if any */
  if ( aSurface->DiscardProc )
    ((XDiscardSurfaceProc)aSurface->DiscardProc)( aSurface, aSurface->Tag1,
                                                  aSurface->Tag2 );
...
{
  /* Depending on the glyph code - select the appropriate hash list */
  unsigned int sel      = aGlyph->GlyphId & ( NoOfGlyphCacheLists - 1 );
...
static void DiscardGlyph( XGlyph* aGlyph )
{
  ...
  #ifdef EW_SUPPORT_GFX_TASK_TRACING
    if ( EwPrintGfxTasks )
      EwPrint( "DiscardGlyph( 0x%p, %d, %d, %d, %d )\n", aGlyph, aGlyph->Width,
                aGlyph->Height, aGlyph->CacheX, aGlyph->CacheY );
  #endif

  /* Remove the glyph from the cache and release the memory area occupied 
     within the glyph surface cache */
  UncacheGlyph( aGlyph );
  UnmapGlyph( aGlyph );
...
static void UnmapGlyph( XGlyph* aGlyph )
{
  ...
  /* Is the glyph actually mapped within the glyph surface? */
  if (( aGlyph->CacheX < 0 ) && ( aGlyph->CacheY < 0 ))
    return;
  ...
  #ifdef EW_SUPPORT_GFX_TASK_TRACING
    if ( EwPrintGfxTasks )
      EwPrint( "UnmapGlyph( 0x%p, %d, %d, %d, %d )\n", aGlyph, aGlyph->Width,
                aGlyph->Height, aGlyph->CacheX, aGlyph->CacheY );
  #endif

  /* Release the area within the surface. */
  ReleaseGlyphCacheArea( col, row, cols, rows );

  /* The glyph does not occupy the glyph surface memory */
  aGlyph->CacheX = -1;
  aGlyph->CacheY = -1;
}
...
/* The following helper function verifies whether the given area is actually
   unused within the glyph cache. */
static int IsGlyphCacheAreaFree( int aColumn, int aRow, int aNoOfColumns, 
  int aNoOfRows )
{
  unsigned int mask = RowMasks[ aNoOfRows ] << ( aRow % 32 );
  int          r    = aRow / 32;

  if (( aRow    < 0 ) || ( aRow    >= NoOfGlyphCacheRows ) ||
      ( aColumn < 0 ) || ( aColumn >= NoOfGlyphCacheCols ))
    return 0;

  for ( ; aNoOfColumns > 0; aColumn++, aNoOfColumns-- )
    if ( EwGlyphCacheSurfaceMap[( aColumn * NoOfGlyphCacheRows / 32 ) + r ] & mask )
      return 0;
...
static void ProcessIssues( void )
{
  ...
  for ( ;; )
  {
    ...
    if ( EwIssuesPending.Tail && !EwIssuesRunning.Tail )
    {
      ...
      /* The state machine is actually processing the issue */
      issue->Busy++;
      ...
      /* The state machine is done with the processing */
      issue->Busy--;
    }
    ...
    /* Any finished issues there? Release all resources used by them and
       recycle it */
    else if ( EwIssuesFinished.Tail && !EwIssuesFinished.Tail->Busy )
    {
...
static int FlushAndClean( int aSurfaces, int aGlyphs, int aMappedGlyphs )
{
  ...
  XGlyph*   glyph   = EwGlyphCacheTail;
  int       count   = aSurfaces + aGlyphs + aMappedGlyphs;
  int       urgency = 0;
  ...
  /* No more issues running or waiting for execution - start destroying unused
     glyphs which occupy the glyph cache surface. In the first pass, promote 
     the glyphs which are nearby free areas. */
  while (( aMappedGlyphs > 0 ) && glyph )
  {
    XGlyph* tmp = glyph;
    int     row;
    int     col;
    ...
    /* Ignore actually used glyphs and glyphs which are not mapped within the
       glyph cache surface */
    if ( tmp->Used || ( tmp->CacheX < 0 ) || ( tmp->CacheY < 0 ))
      continue;

    /* Calculate the size of the area occupied by the glyph ... */
    row = tmp->CacheY / GlyphCacheCellSize;
    col = tmp->CacheX / GlyphCacheCellSize;

    /* ... and take in account an additional one-pixel wide border around the
       glyph */
    #ifdef EW_BORDER_AROUND_GLYPHS
      rows = ( tmp->Height + GlyphCacheCellSize + 1 ) / GlyphCacheCellSize;
      cols = ( tmp->Width  + GlyphCacheCellSize + 1 ) / GlyphCacheCellSize;
    #else
      rows = ( tmp->Height + GlyphCacheCellSize - 1 ) / GlyphCacheCellSize;
      cols = ( tmp->Width  + GlyphCacheCellSize - 1 ) / GlyphCacheCellSize;
    #endif

    /* Try to discard glyphs lying nereaby a free area */
    if (
         ( IsGlyphCacheAreaFree( col, row - 1, cols, 1 )) ||
         ( IsGlyphCacheAreaFree( col - 1, row, 1, rows )) ||
         ( IsGlyphCacheAreaFree( col, row + 1, cols, 1 )) ||
         ( IsGlyphCacheAreaFree( col + 1, row, 1, rows ))
       )
    {
  ...
  /* Now destroy other glyphs which occupy the glyph cache surface. */
  for ( glyph = EwGlyphCacheTail; ( aMappedGlyphs > 0 ) && glyph; )
  {
    XGlyph* tmp = glyph;

    glyph = glyph->Newer;

    /* Discard unused glyphs only and only when they occupy glyph surface */
    if ( !tmp->Used && ( tmp->CacheX >= 0 ) && ( tmp->CacheY >= 0 ))
    {
      UnmapGlyph( tmp );
      aMappedGlyphs--;
  ...
  /* ... continue with all other unused glyphs */
  for ( glyph = EwGlyphCacheTail; ( aGlyphs > 0 ) && glyph; )
  {
    XGlyph* tmp = glyph;
  ...
  for ( ; ( aSurfaces > 0 ) && ( urgency < 3 ); urgency++ )
    for ( surface = EwSurfaceCacheTail; ( aSurfaces > 0 ) && surface; )
    {
      ...
      if ( tmp->Owner && ( tmp->Tag1 || tmp->Tag2 ) && ( tmp->MemSize > 0 ) &&
         ( tmp->Used == 1 ) && ( tmp->Owned == 1 ) && !tmp->Locked &&
  ...
  return count - aSurfaces - aGlyphs - aMappedGlyphs;
}
...
static int CleanSurfaceCache( int aForce )
{
  ...
  while (( aForce || ( EwSurfaceCacheSize > EwMaxSurfaceCacheSize )) && surface )
  {
    ...
    /* Discard unused surfaces only */
    if ( !tmp->Used && (( aForce != 1 ) || !tmp->DoCache ))
    {
      DiscardSurface( tmp );
      count++;
    }
  ...
  for ( ; (( aForce == 2 ) || ( EwSurfaceCacheSize > EwMaxSurfaceCacheSize )) &&
          ( urgency < 3 ); urgency++ )
    for ( surface = EwSurfaceCacheTail; (( aForce == 2 ) || 
