...
void MemManage_Handler(void)
{
  /* USER CODE BEGIN MemoryManagement_IRQn 0 */
	 ...
	 uint32_t cfsr = SCB->CFSR; // Configurable Fault Status Register
	    ...
	    // Log fault details (use a debugger or UART to print)
	    if (cfsr & (1 << 7)) {
	        // MMAR valid flag is set
...
/**
 * @brief  EXTI callback from HAL when
 * @param  GPIO_Pin The pin number that triggered the interrupt.
 */
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{

    if (GPIO_Pin == SYS_PWR_GPIO_PIN)
    {

    	PowerOn_handleIntr( );

    }
    if (GPIO_Pin == HW_QUAD_ENC_B_PIN || GPIO_Pin == GPIO_PIN_7 )
   {
	HwQuadEncoder_incrementPulses( GPIO_Pin );                                               // Count pulses per second
   }
}
