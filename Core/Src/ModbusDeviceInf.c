...
static int16_t ModbusDeviceInf_Write(ModbusRequest_t* pRequest, ModbusResponse_t *pResponse)
{
    ...
    EwPrint( " MbAddress = %d\t size: %d\t data Type: %d Value: %d\r\n",
    		  startAddress,
			  quantity,
			  modbusVars[varIndex].dataType,
			  pRequest->data.writeMultiple.registers[0]);
    if (quantity != modbusVars[varIndex].size)
    {
...
int16_t ModbusDeviceInf_Init(void)
{
    ...
    if (Modbus_Init(MODBUS_CHANNEL_USB) != 0)
        return -1;
    if (Modbus_SetProcessFunction(MODBUS_CHANNEL_USB, ModbusDeviceInf_Process) != 0)
        return -1;
